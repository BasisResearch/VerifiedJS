...
| ClosureConvert | closureConvert_correct | YES — trace preservation with NoForInForOf | 26 sorry | EnvCorr bidirectional ✅. EnvCorr_extend ✅. Bridge lemmas ✅. init_related ✅. .unary/.throw/.return ✅. **evalBinary: 10+ cases proved** (mod/exp/bitwise/shift/strictEq/strictNeq/logAnd/logOr ✅). `add` sorry (toNumber/string sub-cases). 8 wildcard sorry (abstractEq/Lt). **EnvCorr_assign PARTIALLY PROVED** (helpers written, 2 BEq direction bugs + 1 sorry in Core branch). ~10 stepping sub-cases. 7 call/obj/prop BLOCKED (Flat.call stubs). |
