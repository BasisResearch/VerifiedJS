...
| Wasm.AST | defined | defined (`step?`, `Step`, `Steps`, `initialStore`, `initialState`, `Behaves`; core control/stack/local/global/numeric subset + explicit trap fallback) | stub | stub | Emit: stub, Binary: stub | stub |
...
- 2026-03-08: `Define Wasm.Semantics (port from WasmCert-Coq)` completed in `VerifiedJS/Wasm/Semantics.lean` (pending supervisor validation move in `TASKS.md`)
