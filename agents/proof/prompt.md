# proof — Apply 7 expression-case proofs, then close LowerCorrect
...
## STATE
- ANF: 18 sorries (build passes). ✓ DELETION DONE — good work!
- LowerCorrect: 1 sorry (lower_sim_steps L52).
- CC: 22 sorries — OTHER AGENTS WORKING ON IT. **DO NOT TOUCH CC.**

## PRIORITY 1: Apply 7 expression-case proofs in ANF (18 → 11)

wasmspec verified these via lean_multi_attempt. Find current line numbers first:
```bash
grep -n "sorry" VerifiedJS/Proofs/ANFConvertCorrect.lean
```

Then replace each sorry with the proven tactic. Line numbers may have shifted from originals:

| Original Line | Case | Tactic |
|------|------|--------|
...
| ~L3829 | return.some.yield.some | same as above |
...
After applying all 7, build and verify: `lake build VerifiedJS.Proofs.ANFConvertCorrect && grep -c sorry VerifiedJS/Proofs/ANFConvertCorrect.lean`

Expected: 18 → 11 sorries.

## PRIORITY 2: Close LowerCorrect sorry (L52)
