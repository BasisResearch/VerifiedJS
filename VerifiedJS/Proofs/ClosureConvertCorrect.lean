...
private def hasFunctionDef : Core.Expr → Bool
  ...
  | .tryCatch b _ c f => hasFunctionDef b || hasFunctionDef c ||
      match f with | some x => hasFunctionDef x | none => false
...
mutual
/-- If an expression has no `functionDef` sub-nodes, `convertExpr` returns the input state unchanged. -/
private theorem convertExpr_state_id_no_functionDef (e : Core.Expr)
    (scope : List String) (envVar : String) (envMap : Flat.EnvMapping) (st : Flat.CCState)
    (h : hasFunctionDef e = false) :
  ...
  | forOf _ _ _ => simp [Flat.convertExpr]
  | «let» _ init body =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hi := convertExpr_state_id_no_functionDef init scope envVar envMap st h.1
    rw [hi]
    exact convertExpr_state_id_no_functionDef body _ envVar envMap st h.2
  | assign _ value =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef value scope envVar envMap st h
  | «if» cond then_ else_ =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hc := convertExpr_state_id_no_functionDef cond scope envVar envMap st h.1.1
    rw [hc]
    have ht := convertExpr_state_id_no_functionDef then_ scope envVar envMap st h.1.2
    rw [ht]
    exact convertExpr_state_id_no_functionDef else_ scope envVar envMap st h.2
  | seq a b =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have ha := convertExpr_state_id_no_functionDef a scope envVar envMap st h.1
    rw [ha]
    exact convertExpr_state_id_no_functionDef b scope envVar envMap st h.2
  | while_ cond body =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hc := convertExpr_state_id_no_functionDef cond scope envVar envMap st h.1
    rw [hc]
    exact convertExpr_state_id_no_functionDef body scope envVar envMap st h.2
  | call callee args =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hc := convertExpr_state_id_no_functionDef callee scope envVar envMap st h.1
    rw [hc]
    exact convertExprList_state_id_no_functionDef args scope envVar envMap st h.2
  | newObj callee args =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hc := convertExpr_state_id_no_functionDef callee scope envVar envMap st h.1
  ...
  | getProp obj _ =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef obj scope envVar envMap st h
  | deleteProp obj _ =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef obj scope envVar envMap st h
  ...
  | unary _ arg =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef arg scope envVar envMap st h
  | setProp obj _ value =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have ho := convertExpr_state_id_no_functionDef obj scope envVar envMap st h.1
    rw [ho]
    exact convertExpr_state_id_no_functionDef value scope envVar envMap st h.2
  | getIndex obj idx =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have ho := convertExpr_state_id_no_functionDef obj scope envVar envMap st h.1
    rw [ho]
    exact convertExpr_state_id_no_functionDef idx scope envVar envMap st h.2
  | binary _ lhs rhs =>
    simp [hasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExpr]
    have hl := convertExpr_state_id_no_functionDef lhs scope envVar envMap st h.1
    rw [hl]
    exact convertExpr_state_id_no_functionDef rhs scope envVar envMap st h.2
  | setIndex obj idx value =>
    simp only [hasFunctionDef, Bool.or_eq_false_iff] at h
    obtain ⟨⟨ho_h, hi_h⟩, hv_h⟩ := h
    simp only [Flat.convertExpr]
    have ho := convertExpr_state_id_no_functionDef obj scope envVar envMap st ho_h
    rw [ho]
    have hi := convertExpr_state_id_no_functionDef idx scope envVar envMap st hi_h
    rw [hi]
    exact convertExpr_state_id_no_functionDef value scope envVar envMap st hv_h
  | objectLit ps =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertPropList_state_id_no_functionDef ps scope envVar envMap st h
  | arrayLit es =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExprList_state_id_no_functionDef es scope envVar envMap st h
  | throw arg =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef arg scope envVar envMap st h
  | tryCatch body catchParam catchBody finally_ =>
    simp only [hasFunctionDef, Bool.or_eq_false_iff] at h
    obtain ⟨⟨hb_h, hc_h⟩, hf_h⟩ := h
    simp only [Flat.convertExpr]
    have hb := convertExpr_state_id_no_functionDef body scope envVar envMap st hb_h
    rw [hb]
    have hc := convertExpr_state_id_no_functionDef catchBody _ envVar envMap st hc_h
    rw [hc]
    exact convertOptExpr_state_id_no_functionDef finally_ scope envVar envMap st (by
      cases finally_ with
      | none => trivial
      | some f => simp only [Flat.hasFunctionDef_tryCatch_finally] at hf_h; exact hf_h)
  | labeled _ body =>
    simp [hasFunctionDef] at h
    simp only [Flat.convertExpr]
    exact convertExpr_state_id_no_functionDef body scope envVar envMap st h
  | «return» arg =>
    simp only [Flat.convertExpr]
    exact convertOptExpr_state_id_no_functionDef arg scope envVar envMap st (by
      cases arg with
      | none => trivial
      | some e => simp [hasFunctionDef] at h; exact h)
  | yield arg _ =>
    simp only [Flat.convertExpr]
    exact convertOptExpr_state_id_no_functionDef arg scope envVar envMap st (by
      cases arg with
      | none => trivial
      | some e => simp [hasFunctionDef] at h; exact h)
  | functionDef _ _ _ _ _ => simp [hasFunctionDef] at h
  termination_by sizeOf e
  decreasing_by all_goals (try cases ‹Option Core.Expr›) <;> simp_wf <;> omega
private theorem convertExprList_state_id_no_functionDef (es : List Core.Expr)
    (scope : List String) (envVar : String) (envMap : Flat.EnvMapping) (st : Flat.CCState)
    (h : listHasFunctionDef es = false) :
    (Flat.convertExprList es scope envVar envMap st).snd = st := by
  cases es with
  | nil => simp [Flat.convertExprList]
  | cons e rest =>
    simp [listHasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertExprList]
    have he := convertExpr_state_id_no_functionDef e scope envVar envMap st h.1
    rw [he]
    exact convertExprList_state_id_no_functionDef rest scope envVar envMap st h.2
  termination_by sizeOf es
  decreasing_by all_goals (try simp_wf) <;> omega
private theorem convertPropList_state_id_no_functionDef (ps : List (Core.PropName × Core.Expr))
    (scope : List String) (envVar : String) (envMap : Flat.EnvMapping) (st : Flat.CCState)
    (h : propListHasFunctionDef ps = false) :
    (Flat.convertPropList ps scope envVar envMap st).snd = st := by
  cases ps with
  | nil => simp [Flat.convertPropList]
  | cons p rest =>
    obtain ⟨pn, pe⟩ := p
    simp [propListHasFunctionDef, Bool.or_eq_false_iff] at h
    simp only [Flat.convertPropList]
    have he := convertExpr_state_id_no_functionDef pe scope envVar envMap st h.1
    rw [he]
    exact convertPropList_state_id_no_functionDef rest scope envVar envMap st h.2
  termination_by sizeOf ps
  decreasing_by all_goals (try simp_wf) <;> omega
private theorem convertOptExpr_state_id_no_functionDef (oe : Option Core.Expr)
    (scope : List String) (envVar : String) (envMap : Flat.EnvMapping) (st : Flat.CCState)
    (h : match oe with | some e => hasFunctionDef e = false | none => True) :
    (Flat.convertOptExpr oe scope envVar envMap st).snd = st := by
  cases oe with
  | none => simp [Flat.convertOptExpr]
  | some e =>
    simp only [Flat.convertOptExpr]
    exact convertExpr_state_id_no_functionDef e scope envVar envMap st h
  termination_by sizeOf oe
  decreasing_by all_goals simp_all <;> omega
end
        ...
        · -- Error case: let-init evaluates to error.
          ...
          · -- Expression correspondence: use error disjunct (sf'.expr = sa.expr)
            ...
            | inl h_conv =>
              ...
              | _ =>
                -- Non-lit Core result after error: only via tryCatch catch (sorry'd at L8482)
