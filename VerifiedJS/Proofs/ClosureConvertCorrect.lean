...
private theorem Flat_step?_getProp_object (s : Flat.State) (addr : Nat) (prop : Core.PropName) :
    Flat.step? { s with expr := .getProp (.lit (.object addr)) prop } =
      ...
      some (.silent, { expr := .lit v, env := s.env, heap := s.heap,
                       trace := s.trace ++ [.silent], funcs := s.funcs, callStack := s.callStack }) := by
  simp only [Flat.step?, Flat.exprValue?, Flat.step?_pushTrace_expand]; rfl
...
-- setProp: obj is .object addr, value needs stepping
...
-- setProp: obj is non-object value, value needs stepping
...
private theorem closureConvert_step_simulation
    ...
    | some cv =>
      ...
      · -- Object case: heap property lookup
        ...
        · -- ExprAddrWF: coreResult might contain object addrs
          ...
          -- hexprwf : addr < sc.heap.objects.size
          ...
          | some props =>
            ...
            | some kv =>
              exact hheapvwf addr hexprwf props hprops kv (list_find?_mem hfind)
        · -- CCState agreement
  ...
  | setProp obj prop value =>
    ...
    | some cv =>
      ...
      | none =>
        -- value needs stepping: use ih_depth on value
        have hfnv : Flat.exprValue? (Flat.convertExpr value scope envVar envMap st).fst = none :=
        ...
        obtain ⟨sa, hsubstep, hsf'_eq⟩ : ∃ sa,
          ...
          · match hm : Flat.step? { sf with expr := (Flat.convertExpr value scope envVar envMap st).fst } with
            | some (t, sa) =>
              have heq := Flat_step?_setProp_object_step_value sf addr prop _ hfnv t sa hm
              rw [heq] at hstep; simp at hstep
              obtain ⟨rfl, hsf'eq⟩ := hstep
              exact ⟨sa, rfl, hsf'eq.symm⟩
            | none =>
              have heq : Flat.step? { sf with expr := .setProp (.lit (.object addr)) prop
          ...
          · match hm : Flat.step? { sf with expr := (Flat.convertExpr value scope envVar envMap st).fst } with
            | some (t, sa) =>
              ...
              have heq := Flat_step?_setProp_nonobject_step_value sf _ prop _ hfnv hno_flat t sa hm
              rw [heq] at hstep; simp at hstep
              obtain ⟨rfl, hsf'eq⟩ := hstep
              exact ⟨sa, rfl, hsf'eq.symm⟩
            | none =>
              ...
              have heq : Flat.step? { sf with expr := .setProp (.lit (Flat.convertValue cv)) prop
                  (Flat.convertExpr value scope envVar envMap st).fst } = none := by
                cases cv with
                | object addr => exact absurd rfl (hno addr)
                | _ => simp only [Flat.step?, Flat.convertValue, hfnv, hm]
              rw [heq] at hstep; exact absurd hstep (by simp)
        subst hsf'_eq
        ...
        · show Core.step? sc = some (ev, sc')
          have hsc' : sc = { sc with expr := .setProp (.lit cv) prop value } := by
            obtain ⟨_, _, _, _, _, _⟩ := sc; simp only [] at hsc; subst hsc; rfl
          rw [hsc']
          exact Core_step?_setProp_value_step cv prop value hcev_v sc.env sc.heap sc.trace sc.funcs sc.callStack _ _ hcstep_sub
        · simp [sc', htrace, htrace_sub]
        · exact hinj'
        · exact henvCorr'
        · exact henvwf'
        · exact hheapvwf'
        · simp [sc', noCallFrameReturn]; exact ⟨by simp [noCallFrameReturn] at hncfr; exact hncfr.1, hncfr'⟩
        · simp only [sc', ExprAddrWF]; exact ⟨by
              simp [ValueAddrWF]; cases cv with
              | object addr =>
                simp [ExprAddrWF, ValueAddrWF] at hexprwf
                exact Nat.lt_of_lt_of_le hexprwf.1 (Core_step_heap_size_mono hcstep_sub)
              | _ => trivial,
            hexprwf'⟩
        · have hval := convertExpr_state_determined value scope envVar envMap st st_a' hAgreeOut.1 hAgreeOut.2
          ...
          · rw [hst]; exact hval.2
      | some vv =>
        -- Both obj and value are values: execute setProp directly
        have hlit_v : value = .lit vv := by
          cases value <;> simp [Core.exprValue?] at hcev_v; subst hcev_v; rfl
        subst hlit_v
        simp [Flat.convertExpr] at hfexpr hst
        -- Now sf.expr = .setProp (.lit (convertValue cv)) prop (.lit (convertValue vv))
        have hsf_eta2 : sf = { sf with expr := .setProp (.lit (Flat.convertValue cv)) prop (.lit (Flat.convertValue vv)) } := by
          cases sf; simp_all
        ...
        have hno_core : (∃ addr, cv = .object addr) ∨ (∀ a, cv ≠ .object a) := by
          ...
          | _ => right; intro a; exact Core.Value.noConfusion
        rcases hno_core with ⟨addr, rfl⟩ | hno
        · -- Object case: setProp on heap object
          have : Flat.convertValue (.object addr) = .object addr := rfl
          rw [this] at hstep hsf_eta2 hfexpr
          ...
          · simp [sc', htrace]
          · -- HeapInj: need to show Flat heap' = Core heap' via HeapInj
            simp only [sc', coreHeap']
            rw [heapObjectAt?_eq, ← HeapInj_get hinj haddr_wf, flatToCoreValue_convertValue]
            cases sc.heap.objects[addr]? with
            | none => exact hinj
          ...
          · -- HeapValuesWF: updated list has valid addresses
            simp only [sc', coreHeap']
            cases hprops : sc.heap.objects[addr]? with
            | none => exact hheapvwf
            | some props =>
              exact HeapValuesWF_set_at hheapvwf (fun kv hkv => by
                -- kv is in the updated list: either original or (prop, vv)
                simp only at hkv
                split at hkv
                · -- map case: kv is mapped from some orig
                  obtain ⟨orig, horig, heq⟩ := List.mem_map.mp hkv
                  split at heq
                  · subst heq; simp [ValueAddrWF]
                    exact (by simp [ExprAddrWF, ValueAddrWF] at hexprwf; exact hexprwf.2)
                  · subst heq
                    exact ValueAddrWF_mono
                      (hheapvwf addr haddr_wf props hprops orig horig)
                      (by simp [size_set!])
                · -- append case: kv ∈ props or kv = (prop, vv)
                  rcases List.mem_append.mp hkv with h | h
                  · exact ValueAddrWF_mono
                      (hheapvwf addr haddr_wf props hprops kv h)
                      (by simp [size_set!])
                  · simp at h; obtain ⟨_, rfl⟩ := h; simp [ValueAddrWF]
                    exact (by simp [ExprAddrWF, ValueAddrWF] at hexprwf; exact hexprwf.2))
          · simp [sc', noCallFrameReturn]
        ...
        · -- Non-object case: return value, no heap change
          have hno_flat := convertValue_not_object cv hno
