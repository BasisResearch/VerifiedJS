  ...
  | tryCatch body catchParam catchBody finally_ =>
    ...
    | none =>
      ...
      have hfnv : Flat.exprValue? fbody = none :=
        convertExpr_not_value_supported body hbv (by revert hsupp; cases finally_ <;> simp [Core.Expr.supported, Bool.and_eq_true] <;> tauto) scope envVar envMap st
        ...
        · -- Error: catch clause activated (scope mismatch blocks full proof)
          ...
          obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf',
            ...
            ih_depth body.depth hdepth envVar envMap injMap
              ...
              (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_body hexprwf_body (by revert hsupp; cases finally_ <;> simp [Core.Expr.supported, Bool.and_eq_true] <;> tauto)
              (by simp [fbody, st1])
              ⟨hm⟩
