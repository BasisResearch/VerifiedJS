      ...
      | some iv =>
        ...
        | some vv =>
          ...
          · -- Object case: heap mutation
            ...
            -- Build core result state (same pattern as setProp object both values)
            let coreHeap' := match sc.heap.objects[addr]? with
              | some (props : List (Core.PropName × Core.Value)) =>
            ...
            · -- Core step
              ...
              simp only [Core.pushTrace, sc', coreHeap'] at this ⊢; exact this
            · simp [sc', htrace]
            · -- HeapInj: same pattern as setProp
              simp only [sc', coreHeap']
              rw [heapObjectAt?_eq, ← HeapInj_get hinj haddr_wf]
              cases sc.heap.objects[addr]? with
              | none => exact hinj
        ...
        | none =>
          ...
          have hfnv_v : Flat.exprValue? (Flat.convertExpr value scope envVar envMap
              (Flat.convertExpr idx scope envVar envMap st).snd).fst = none :=
            convertExpr_not_value value hcev_v scope envVar envMap _
          -- Extract value sub-step from Flat step
          have hno_core : (∃ addr, cv = .object addr) ∨ (∀ a, cv ≠ .object a) := by
            cases cv with
            | object a => left; exact ⟨a, rfl⟩
            | _ => right; intro a; exact Core.Value.noConfusion
          obtain ⟨sa, hsubstep, hsf'_eq⟩ : ∃ sa,
              Flat.step? { sf with expr := (Flat.convertExpr value scope envVar envMap
                (Flat.convertExpr idx scope envVar envMap st).snd).fst } = some (ev, sa) ∧
              sf' = { expr := .setIndex (.lit (Flat.convertValue cv)) (.lit (Flat.convertValue iv)) sa.expr,
                      env := sa.env, heap := sa.heap,
                      trace := sf.trace ++ [ev], funcs := sf.funcs, callStack := sf.callStack } := by
            match hm : Flat.step? { sf with expr := (Flat.convertExpr value scope envVar envMap
                (Flat.convertExpr idx scope envVar envMap st).snd).fst } with
            | some (t, sa) =>
              rcases hno_core with ⟨addr, rfl⟩ | hno
              · have : Flat.convertValue (.object addr) = .object addr := rfl
                rw [this] at hstep
            ...
            | none =>
              ...
              · have : Flat.convertValue (.object addr) = .object addr := rfl
                rw [this] at hstep
                simp only [Flat.step?, Flat.exprValue?, hfnv_v, hm] at hstep
              ...
              · have hno_flat := convertValue_not_object cv hno
                ...
                cases hcv : Flat.convertValue cv <;> simp_all [Flat.exprValue?, hfnv_v, hm]
          subst hsf'_eq
          have hdepth : value.depth < n := by simp [Core.Expr.depth] at hd; omega
          have hncfr_v : noCallFrameReturn value = true := by
          ...
          obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
            ih_depth value.depth hdepth envVar envMap injMap
              { sf with expr := (Flat.convertExpr value scope envVar envMap
                (Flat.convertExpr idx scope envVar envMap st).snd).fst }
              { sc with expr := value }
              ev sa scope (Flat.convertExpr idx scope envVar envMap st).snd
              (Flat.convertExpr value scope envVar envMap (Flat.convertExpr idx scope envVar envMap st).snd).snd
              (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_v hexprwf_v
              (by simp)
              ⟨hsubstep⟩
          let sc' : Core.State :=
          ...
          · show Core.step? sc = some (ev, sc')
            have hsc' : sc = { sc with expr := .setIndex (.lit cv) (.lit iv) value } := by
              obtain ⟨_, _, _, _, _, _⟩ := sc; simp only [] at hsc; subst hsc; rfl
            rw [hsc']
            exact Core_step?_setIndex_value_step_value cv iv _ hcev_v _ _ _ _ _ _ _ hcstep_sub
          · simp [sc', htrace, htrace_sub]
          · exact hinj'
          · exact henvCorr'
          · exact henvwf'
          ...
          · simp only [sc']; simp only [ExprAddrWF]; exact ⟨hexprwf', by
              constructor
              · exact ValueAddrWF_mono (by simp [ExprAddrWF, ValueAddrWF] at hexprwf; exact hexprwf.1) (Core_step_heap_size_mono hcstep_sub)
              · exact ExprAddrWF_mono iv (by simp [ExprAddrWF] at hexprwf; exact hexprwf.2.1) (Core_step_heap_size_mono hcstep_sub)⟩
          · have hval := convertExpr_state_determined value scope envVar envMap
                (Flat.convertExpr idx scope envVar envMap st).snd st_a' hAgreeOut.1 hAgreeOut.2
            refine ⟨st_a, (Flat.convertExpr value scope envVar envMap st_a').snd, ?_, hAgreeIn, ?_⟩
            · simp only [sc', Flat.convertExpr]
              rw [show (Flat.convertExpr sc_sub'.expr scope envVar envMap st_a).fst = sa.expr from (congrArg Prod.fst hconv').symm]
              rw [show (Flat.convertExpr sc_sub'.expr scope envVar envMap st_a).snd = st_a' from (congrArg Prod.snd hconv').symm]
      ...
      | none =>
        ...
        · show Core.step? sc = some (ev, sc')
          have hsc' : sc = { sc with expr := .setIndex (.lit cv) idx value } := by
            obtain ⟨_, _, _, _, _, _⟩ := sc; simp only [] at hsc; subst hsc; rfl
          rw [hsc']
          exact Core_step?_setIndex_value_step_idx cv _ _ hcev_i _ _ _ _ _ _ _ hcstep_sub
        · simp [sc', htrace, htrace_sub]
        · exact hinj'
        · exact henvCorr'
        · exact henvwf'
        ...
        · simp only [sc']; simp only [ExprAddrWF]; constructor
          · exact ⟨ValueAddrWF_mono (by simp [ExprAddrWF, ValueAddrWF] at hexprwf; exact hexprwf.1) (Core_step_heap_size_mono hcstep_sub), hexprwf'⟩
    ...
    | some val =>
      ...
      · -- CCState agreement (objectLit sub-step)
        ...
        have htgt_eq : (Flat.convertExpr sc_sub'.expr scope envVar envMap
          ...
          hsd.1.trans hconv'_fst.symm
        -- CCStateAgree between old target output and new target output
        have hagree_mid : CCStateAgree
            (Flat.convertExpr target_c scope envVar envMap
              (Flat.convertPropList done_c scope envVar envMap st).snd).snd
            (Flat.convertExpr sc_sub'.expr scope envVar envMap
              (Flat.convertPropList done_c scope envVar envMap st).snd).snd := by
          constructor
          · have h1 := hAgreeOut.1; rw [hconv'_snd] at h1; exact h1.trans hsd.2.1.symm
          · have h2 := hAgreeOut.2; rw [hconv'_snd] at h2; exact h2.trans hsd.2.2.symm
        -- State determination for rest_c with agreeing intermediate states
        have hsd_rest := convertPropList_state_determined rest_c scope envVar envMap
          _ _ hagree_mid.1 hagree_mid.2
        -- Helpers for singleton convertPropList
        have hcpls_snd : ∀ (k : Core.PropName) (e : Core.Expr) (s : Flat.CCState),
            (Flat.convertPropList [(k, e)] scope envVar envMap s).snd =
            (Flat.convertExpr e scope envVar envMap s).snd := by
          intro k e s; simp [Flat.convertPropList]
        have hcpls_fst : ∀ (k : Core.PropName) (e : Core.Expr) (s : Flat.CCState),
            (Flat.convertPropList [(k, e)] scope envVar envMap s).fst =
            [(k, (Flat.convertExpr e scope envVar envMap s).fst)] := by
          intro k e s; simp [Flat.convertPropList]
        -- Decompose convertPropList for done ++ [(k, x)] ++ rest
        have hdecomp_fst : ∀ (x : Core.Expr),
            (Flat.convertPropList (done_c ++ [(propName_c, x)] ++ rest_c) scope envVar envMap st).fst =
            (Flat.convertPropList done_c scope envVar envMap st).fst ++
            [(propName_c, (Flat.convertExpr x scope envVar envMap
              (Flat.convertPropList done_c scope envVar envMap st).snd).fst)] ++
            (Flat.convertPropList rest_c scope envVar envMap
              (Flat.convertExpr x scope envVar envMap
                (Flat.convertPropList done_c scope envVar envMap st).snd).snd).fst := by
          intro x
          rw [convertPropList_append, convertPropList_append, hcpls_fst]
          rw [show (Flat.convertPropList (done_c ++ [(propName_c, x)]) scope envVar envMap st).snd =
              (Flat.convertExpr x scope envVar envMap
                (Flat.convertPropList done_c scope envVar envMap st).snd).snd from by
            rw [convertPropList_append_snd, hcpls_snd]]
        have hdecomp_snd : ∀ (x : Core.Expr),
            (Flat.convertPropList (done_c ++ [(propName_c, x)] ++ rest_c) scope envVar envMap st).snd =
            (Flat.convertPropList rest_c scope envVar envMap
              (Flat.convertExpr x scope envVar envMap
                (Flat.convertPropList done_c scope envVar envMap st).snd).snd).snd := by
          intro x
          rw [convertPropList_append_snd, convertPropList_append_snd, hcpls_snd]
        -- Expression equality after conversion
        have hexpr_eq :
            Flat.Expr.objectLit ((Flat.convertPropList done_c scope envVar envMap st).fst ++
              [(propName_c, sa.expr)] ++
              (Flat.convertPropList rest_c scope envVar envMap
                (Flat.convertExpr target_c scope envVar envMap
                  (Flat.convertPropList done_c scope envVar envMap st).snd).snd).fst) =
            Flat.Expr.objectLit (Flat.convertPropList (done_c ++ [(propName_c, sc_sub'.expr)] ++ rest_c) scope envVar envMap st).fst := by
          rw [hdecomp_fst]
          congr 1
          congr 1
          · congr 1
            congr 1
            · exact Prod.ext rfl htgt_eq.symm
          · exact hsd_rest.1
        refine ⟨st, (Flat.convertPropList (done_c ++ [(propName_c, sc_sub'.expr)] ++ rest_c) scope envVar envMap st).snd,
          ?_, ⟨rfl, rfl⟩, ?_⟩
        · -- Pair equality
          have : Flat.convertExpr (Core.Expr.objectLit (done_c ++ [(propName_c, sc_sub'.expr)] ++ rest_c)) scope envVar envMap st =
              (Flat.Expr.objectLit (Flat.convertPropList (done_c ++ [(propName_c, sc_sub'.expr)] ++ rest_c) scope envVar envMap st).fst,
               (Flat.convertPropList (done_c ++ [(propName_c, sc_sub'.expr)] ++ rest_c) scope envVar envMap st).snd) := by
            simp [Flat.convertExpr]
          rw [this]
          exact Prod.ext hexpr_eq rfl
        · -- Output CCState agreement
          rw [hst, hprops, hdecomp_snd target_c, hdecomp_snd sc_sub'.expr]
          exact hsd_rest.2
  | arrayLit elems =>
    rw [hsc] at hconv hncfr hexprwf hd
    simp [Flat.convertExpr] at hconv
  ...
  | tryCatch body catchParam catchBody finally_ =>
    ...
    | none =>
      ...
      have hsf_eta : sf = { sf with expr := .tryCatch fbody catchParam fcatch ffin } := by
        cases sf; simp_all
      rw [hsf_eta] at hstep
      -- Case split on whether the trace event is an error
      have hev_split : (∃ msg, ev = .error msg) ∨ (∀ msg, ev ≠ .error msg) := by
        cases ev with
        | error msg => left; exact ⟨msg, rfl⟩
        | _ => right; intro msg h; cases h
      rcases hev_split with ⟨msg, rfl⟩ | hne
      · -- Error case: body step produced error, catch handler activates
        sorry -- CCState threading: catch body compiled at state st1 but need CCStateAgree st st_a
      · -- Non-error case: body step propagated through tryCatch wrapper
        obtain ⟨sa, hsubstep, hsf'_eq⟩ : ∃ sa,
            Flat.step? { sf with expr := fbody } = some (ev, sa) ∧
            sf' = { expr := .tryCatch sa.expr catchParam fcatch ffin,
                    env := sa.env, heap := sa.heap,
                    trace := sf.trace ++ [ev], funcs := sf.funcs, callStack := sf.callStack } := by
          match hm : Flat.step? { sf with expr := fbody } with
          | some (t, sa) =>
            have heq := Flat_step?_tryCatch_body_step sf fbody catchParam fcatch ffin sa t hncf hfnv hm hne
            rw [heq] at hstep; simp at hstep
            obtain ⟨rfl, hsf'eq⟩ := hstep
            exact ⟨sa, rfl, hsf'eq.symm⟩
          | none =>
        ...
        refine ⟨injMap', sc', ⟨?_⟩, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
        · show Core.step? sc = some (ev, sc')
          have hsc' : sc = { sc with expr := .tryCatch body catchParam catchBody finally_ } := by
            obtain ⟨_, _, _, _, _, _⟩ := sc; simp only [] at hsc; subst hsc; rfl
          rw [hsc']
        ...
        · simp [sc', noCallFrameReturn]
          simp [noCallFrameReturn] at hncfr
          exact ⟨hncf, hncfr', hncfr.2.2.1, hncfr.2.2.2⟩
        · simp only [sc', ExprAddrWF]
          have hmono := Core_step_heap_size_mono hcstep_sub
          simp [ExprAddrWF] at hexprwf
          cases finally_ with
          | none => exact ⟨hexprwf', ExprAddrWF_mono catchBody hexprwf.2.1 hmono⟩
          | some fin => exact ⟨hexprwf', ExprAddrWF_mono catchBody hexprwf.2.1 hmono, ExprAddrWF_mono fin hexprwf.2.2 hmono⟩
        · -- CCState threading: body stepped, catch and finally unchanged
          have hcatch := convertExpr_state_determined catchBody (catchParam :: scope) envVar envMap
              st1 st_a' hAgreeOut.1 hAgreeOut.2
          have hfin := convertOptExpr_state_determined finally_ scope envVar envMap
              st2 (Flat.convertExpr catchBody (catchParam :: scope) envVar envMap st_a').snd
              hcatch.2.1 hcatch.2.2
          refine ⟨st_a,
