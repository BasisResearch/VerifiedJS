...
private theorem Flat_step?_call_value_step_arg (s : Flat.State)
    ...
    Flat.step? { s with expr := .call (.lit fv) (.lit ev) args } =
      some (t, { expr := .call (.lit fv) (.lit ev) (done ++ [sa.expr] ++ remaining),
                 ...
                 trace := s.trace ++ [t], funcs := s.funcs, callStack := s.callStack }) := by
  unfold Flat.step?
  simp only [Flat.exprValue?, hvals, hfnv, hss]
...
private theorem Flat_step?_call_nonclosure (s : Flat.State)
  ...
  | _ => unfold Flat.step?; simp only [Flat.exprValue?, hvals]
...
private theorem closureConvert_step_simulation
  ...
  | call f args =>
    ...
    | some cv =>
      ...
      | some argVals =>
        sorry -- all args are values: actual call execution (function or non-function)
      | none =>
        ...
        | some val =>
          ...
          have hffnv := convertExprList_firstNonValueExpr_some args scope envVar envMap st
              done_c target_c rest_c hfnv_args htarget_novalue
          ...
          obtain ⟨sa, hsubstep, hsf'_eq⟩ : ∃ sa,
            ...
            | some (t, se) =>
              have heq := Flat_step?_call_value_step_arg sf (Flat.convertValue cv) .null
                (Flat.convertExprList args scope envVar envMap st).fst hvals
                (Flat.convertExprList done_c scope envVar envMap st).fst
                (Flat.convertExpr target_c scope envVar envMap
          ...
          have hdepth : target_c.depth < n := by
            ...
            have := Core.firstNonValueExpr_depth hfnv_args; omega
          have ⟨hncfr_done, hncfr_target, hncfr_rest⟩ :=
            firstNonValueExpr_listNoCallFrameReturn hfnv_args (by simp [noCallFrameReturn] at hncfr; exact hncfr)
          have hexprwf_target : ExprAddrWF target_c sc.heap.objects.size := by
            simp [ExprAddrWF] at hexprwf
            exact ExprAddrListWF_firstNonValueExpr_target hfnv_args hexprwf
          obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf',
                  hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
          ...
          · -- Core.step?
            ...
            have hsc' : sc = { sc with expr := .call (.lit cv) args } := by
              obtain ⟨_, _, _, _, _, _⟩ := sc; simp only [] at hsc; subst hsc; rfl
            rw [hsc']
            exact Core.step_call_step_arg cv args sc.env sc.heap sc.trace sc.funcs sc.callStack
              hallv done_c target_c rest_c hfnv_args ev sc_sub' (by obtain ⟨_, _, _, _, _, _⟩ := sc; exact hcstep_sub)
          · -- trace
            simp [sc', htrace, htrace_sub]
          · exact hinj'
          · exact henvCorr'
          ...
          · -- noCallFrameReturn
            simp [sc', noCallFrameReturn]
            exact ⟨hncfr', hncfr_done, hncfr_rest⟩
          · -- ExprAddrWF
            simp only [sc', ExprAddrWF, ValueAddrWF]
            exact ⟨by simp [ExprAddrWF] at hexprwf; exact hexprwf.1,
                   hexprwf',
                   ExprAddrListWF_firstNonValueExpr_reconstruct hfnv_args
                     (by simp [ExprAddrWF] at hexprwf; exact hexprwf.2)
                     hexprwf' (Core_step_heap_size_mono hcstep_sub)⟩
          · -- CC state
            have hargs := convertExprList_state_determined args scope envVar envMap
                st st_a hAgreeIn.1 hAgreeIn.2
            refine ⟨st_a, (Flat.convertExprList (done_c ++ [sc_sub'.expr] ++ rest_c) scope envVar envMap st_a).snd, ?_, hAgreeIn, ?_⟩
