...
private theorem closureConvert_step_simulation
  ...
  | «if» cond then_ else_ =>
    rw [hsc] at hconv; simp only [Flat.convertExpr] at hconv
    have hsf_expr : sf.expr = .«if» (Flat.convertExpr cond scope envVar envMap st).fst
        (Flat.convertExpr then_ scope envVar envMap (Flat.convertExpr cond scope envVar envMap st).snd).fst
        (Flat.convertExpr else_ scope envVar envMap (Flat.convertExpr then_ scope envVar envMap (Flat.convertExpr cond scope envVar envMap st).snd).snd).fst := by
      cases sf; simp_all [(Prod.mk.inj hconv).1]
    cases hval : Core.exprValue? cond with
    | some v =>
      have ha_lit : cond = .lit v := by cases cond <;> simp [Core.exprValue?] at hval <;> exact congrArg _ hval
      subst ha_lit
      simp only [Flat.convertExpr] at hsf_expr hconv
      -- Both branch on toBoolean v (which equals Flat.toBoolean (convertValue v))
      have hsf_rw : sf = ⟨Flat.Expr.«if» (.lit (Flat.convertValue v))
          (Flat.convertExpr then_ scope envVar envMap st).fst
          (Flat.convertExpr else_ scope envVar envMap (Flat.convertExpr then_ scope envVar envMap st).snd).fst,
      ...
      refine ⟨sc', ⟨hcstep⟩, ?_⟩
      have hsf'_trace : sf'.trace = sc'.trace := by
        have hf := hstep; have hc := hcstep; rw [hsf_rw] at hf; rw [hsc_rw] at hc
        simp only [Flat.step?, Flat.exprValue?] at hf; simp only [Core.step?, Core.exprValue?] at hc
        split at hf <;> next =>
          have heqf := (Prod.mk.inj (Option.some.inj hf)).2
          have heqc := (Prod.mk.inj (Option.some.inj hc)).2
          subst heqf; subst heqc; show sf.trace ++ _ = sc.trace ++ _; rw [htrace]
      have henv' : EnvCorr sc'.env sf'.env := by
        have hsf'_env : sf'.env = sf.env := by
          have h0 := hstep; rw [hsf_rw] at h0; simp only [Flat.step?, Flat.exprValue?] at h0
          split at h0 <;> next => have heq := (Prod.mk.inj (Option.some.inj h0)).2; subst heq; rfl
        have hsc'_env : sc'.env = sc.env := by
          have h0 := hcstep; rw [hsc_rw] at h0; simp only [Core.step?, Core.exprValue?] at h0
          have heq := (Prod.mk.inj (Option.some.inj h0)).2; subst heq; rfl
        rw [hsc'_env, hsf'_env]; exact henvCorr
      -- Expression correspondence: depends on which branch was taken
      cases hbool : Core.toBoolean v with
      | true =>
        -- Both take then_ branch
        have hsf'_expr : sf'.expr = (Flat.convertExpr then_ scope envVar envMap st).fst := by
          have h0 := hstep; rw [hsf_rw] at h0; simp only [Flat.step?, Flat.exprValue?] at h0
          rw [show Flat.toBoolean (Flat.convertValue v) = true from by rw [toBoolean_convertValue, hbool]] at h0
          simp at h0
          exact congrArg Flat.State.expr (Prod.mk.inj (Option.some.inj h0)).2 ▸ rfl
        have hsc'_expr : sc'.expr = then_ := by
          have h0 := hcstep; rw [hsc_rw] at h0; simp only [Core.step?, Core.exprValue?] at h0
          rw [hbool] at h0; simp at h0
          exact congrArg Core.State.expr (Prod.mk.inj (Option.some.inj h0)).2 ▸ rfl
        exact ⟨hsf'_trace, henv', scope, envVar, envMap, st,
          (Flat.convertExpr then_ scope envVar envMap st).snd,
          by rw [hsc'_expr]; simp [Flat.convertExpr, hsf'_expr]⟩
      | false =>
        -- Both take else_ branch
        have hsf'_expr : sf'.expr = (Flat.convertExpr else_ scope envVar envMap (Flat.convertExpr then_ scope envVar envMap st).snd).fst := by
          have h0 := hstep; rw [hsf_rw] at h0; simp only [Flat.step?, Flat.exprValue?] at h0
          rw [show Flat.toBoolean (Flat.convertValue v) = false from by rw [toBoolean_convertValue, hbool]] at h0
          simp at h0
          exact congrArg Flat.State.expr (Prod.mk.inj (Option.some.inj h0)).2 ▸ rfl
        have hsc'_expr : sc'.expr = else_ := by
          have h0 := hcstep; rw [hsc_rw] at h0; simp only [Core.step?, Core.exprValue?] at h0
          rw [hbool] at h0; simp at h0
          exact congrArg Core.State.expr (Prod.mk.inj (Option.some.inj h0)).2 ▸ rfl
        exact ⟨hsf'_trace, henv', scope, envVar, envMap,
          (Flat.convertExpr then_ scope envVar envMap st).snd,
          (Flat.convertExpr else_ scope envVar envMap (Flat.convertExpr then_ scope envVar envMap st).snd).snd,
          by rw [hsc'_expr]; simp [Flat.convertExpr, hsf'_expr]⟩
    | none =>
      sorry -- stepping sub-case
  | seq a b =>
    rw [hsc] at hconv; simp only [Flat.convertExpr] at hconv
