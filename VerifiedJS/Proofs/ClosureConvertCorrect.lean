  ...
  | assign name rhs =>
    ...
    | none =>
      have hfnv : Flat.exprValue? (Flat.convertExpr rhs scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported rhs hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth rhs.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_rhs hexprwf_rhs (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
  ...
  | seq a b =>
    ...
    | none =>
      have hfnv : Flat.exprValue? (Flat.convertExpr a scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported a hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth a.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_a hexprwf_a (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
  ...
  | unary op arg =>
    ...
    | none =>
      have hfnv : Flat.exprValue? (Flat.convertExpr arg scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported arg hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth arg.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_arg hexprwf_arg (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
  ...
  | binary op lhs rhs =>
    ...
    | none =>
      ...
      have hfnv : Flat.exprValue? (Flat.convertExpr lhs scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported lhs hlv (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth lhs.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_lhs hexprwf_lhs (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
  ...
  | call f args =>
    ...
    | none =>
      have hfnv : Flat.exprValue? (Flat.convertExpr f scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported f hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf',
        ...
        ih_depth f.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_f hexprwf_f (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
      ...
      | none =>
        ...
        | some val =>
          ...
          have htarget_supp : target_c.supported = true :=
            listSupported_firstNonValueExpr_target hcfnv (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
  ...
  | getProp obj prop =>
    ...
    | none =>
      have hfnv : Flat.exprValue? (Flat.convertExpr obj scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported obj hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
    ...
    | some cv =>
      ...
      | none =>
        ...
        have hfnv_v : Flat.exprValue? (Flat.convertExpr value scope envVar envMap st).fst = none :=
          convertExpr_not_value_supported value hcev_v (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
        ...
        obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf',
          ...
          ih_depth value.depth hdepth envVar envMap injMap
            ...
            (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_v hexprwf_v (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
            (by simp)
            ⟨hsubstep⟩
        let sc' : Core.State :=
    ...
    | none =>
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth obj.depth hdepth envVar envMap injMap
          ...
          { sc with expr := obj }
          ev sa scope st (Flat.convertExpr obj scope envVar envMap st).snd
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_obj hexprwf_obj (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
      let sc' : Core.State :=
    ...
    | some cv =>
      ...
      | none =>
        ...
        have hfnv_i : Flat.exprValue? (Flat.convertExpr idx scope envVar envMap st).fst = none :=
          convertExpr_not_value_supported idx hcev_i (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
        -- Extract value sub-step from Flat step
        obtain ⟨sa, hsubstep, hsf'_eq⟩ : ∃ sa,
        ...
        obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf',
          ...
          ih_depth idx.depth hdepth envVar envMap injMap
            ...
            (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_i hexprwf_i (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
            (by simp)
            ⟨hsubstep⟩
        let sc' : Core.State :=
  ...
  | objectLit props =>
    ...
    | some val =>
      ...
      have htarget_novalue : Core.exprValue? target_c = none := by
        ...
        | _ => rfl
      have htarget_supp : target_c.supported = true :=
        propListSupported_firstNonValueProp_target hcfnv (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
  ...
  | throw val =>
    ...
    | none =>
      ...
      have hfnv : Flat.exprValue? (Flat.convertExpr val scope envVar envMap st).fst = none :=
        convertExpr_not_value_supported val hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
      ...
      obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf', hheapna', hncfr', hexprwf', st_a, st_a', hconv', hAgreeIn, hAgreeOut⟩ :=
        ih_depth val.depth hdepth envVar envMap injMap
          ...
          (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_val hexprwf_val (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
          (by simp)
          ⟨hsubstep⟩
  ...
  | tryCatch body catchParam catchBody finally_ =>
    ...
    | none =>
      ...
      have hfnv : Flat.exprValue? fbody = none :=
        convertExpr_not_value_supported body hbv (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
        ...
        · -- Error: catch clause activated (scope mismatch blocks full proof)
          ...
          obtain ⟨injMap', sc_sub', ⟨hcstep_sub⟩, htrace_sub, hinj', henvCorr', henvwf', hheapvwf',
            ...
            ih_depth body.depth hdepth envVar envMap injMap
              ...
              (by simp [Core.Expr.depth]) htrace hinj henvCorr henvwf hheapvwf hheapna hncfr_body hexprwf_body (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h)
              (by simp [fbody, st1])
              ⟨hm⟩
    ...
    | some e =>
      ...
      | none =>
        ...
        have hfnv : Flat.exprValue? (Flat.convertExpr e scope envVar envMap st).fst = none :=
          convertExpr_not_value_supported e hcev (by have h := hsupp; unfold Core.Expr.supported at h; try simp only [Bool.and_eq_true] at h; exact h) scope envVar envMap st
