...
private theorem hasAbruptCompletion_step_preserved (e : Flat.Expr)
  ...
  induction n with
  | zero =>
    intro e hd env heap trace funcs cs ev sf' hac hfuncs_ac hstep
    cases e with
    | lit v => simp [Flat.step?] at hstep
    | var name => unfold Flat.step? at hstep; split at hstep <;> simp at hstep <;> obtain ⟨_, rfl⟩ := hstep <;> simp [Flat.State.expr, hasAbruptCompletion]
    | this => unfold Flat.step? at hstep; split at hstep <;> simp at hstep <;> obtain ⟨_, rfl⟩ := hstep <;> simp [Flat.State.expr, hasAbruptCompletion]
    | «break» _ => simp [hasAbruptCompletion] at hac
    | «continue» _ => simp [hasAbruptCompletion] at hac
    | throw _ => simp [hasAbruptCompletion] at hac
    | «return» _ => simp [hasAbruptCompletion] at hac
    | await _ => simp [hasAbruptCompletion] at hac
    | yield _ _ => simp [hasAbruptCompletion] at hac
  ...
  | succ n ih =>
    ...
    | unary op arg =>
      ...
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hsa
        next => simp at hstep
    | binary op lhs rhs =>
      ...
      unfold Flat.step? at hstep
      split at hstep
      next =>  -- lhs not value → step lhs
        split at hstep
        next ev' sl hsl => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hl hfuncs_ac hsl, hr⟩
        next => simp at hstep
      next lv =>  -- lhs is value
        split at hstep
        next =>  -- rhs not value → step rhs
          split at hstep
          next ev' sr hsr => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨trivial, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hr hfuncs_ac hsr⟩
          next => simp at hstep
        next rv =>  -- both values
    ...
    | while_ cond body =>
      ...
      obtain ⟨hc, hb⟩ := hac
      unfold Flat.step? at hstep
      simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion, hc, hb]
    | labeled label body =>
      simp only [hasAbruptCompletion] at hac
      unfold Flat.step? at hstep
      simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact hac
    | seq a b =>
      simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
      obtain ⟨ha, hb⟩ := hac
      unfold Flat.step? at hstep
      split at hstep
      next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact hb
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ ha hfuncs_ac hsa, hb⟩
        next => simp at hstep
    | «let» name init body =>
      simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
      obtain ⟨hi, hb⟩ := hac
      ...
      next hv =>
        split at hstep
        next ev' si hsi => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hi hfuncs_ac hsi, hb⟩
        next => simp at hstep
    | assign name rhs =>
      simp only [hasAbruptCompletion] at hac
      unfold Flat.step? at hstep
      ...
      next hv =>
        split at hstep
        next ev' sr hsr => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hsr
        next => simp at hstep
    | «if» cond then_ else_ =>
      simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
      obtain ⟨⟨hc, ht⟩, he⟩ := hac
      unfold Flat.step? at hstep
      split at hstep
      next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; split <;> assumption
      next hv =>
        split at hstep
        next ev' sc hsc => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hc hfuncs_ac hsc, ht⟩, he⟩
        next => simp at hstep
    | getProp obj prop =>
      simp only [hasAbruptCompletion] at hac
      unfold Flat.step? at hstep
      ...
      next hv =>
        split at hstep
        next ev' so hso => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hso
        next => simp at hstep
    | setProp obj prop val =>
      simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
      obtain ⟨ho, hv⟩ := hac
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ ho hfuncs_ac hso, hv⟩
        next => simp at hstep
      next addr =>  -- obj = .object addr
        ...
        next =>  -- val not value → step val
          split at hstep
          next ev' sv hsv => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ho, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hv hfuncs_ac hsv⟩
          next => simp at hstep
      next =>  -- obj = some other value
        split at hstep
        next vv =>  -- val is value
          simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
        next =>  -- val not value → step val
          split at hstep
          next ev' sv hsv => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ho, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hv hfuncs_ac hsv⟩
          next => simp at hstep
    | getIndex obj idx =>
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ ho hfuncs_ac hso, hi⟩
        next => simp at hstep
      next addr =>  -- obj = .object addr
        split at hstep
        ...
        next =>  -- idx not value → step idx
          split at hstep
          next ev' si hsi => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨ho, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hi hfuncs_ac hsi⟩
          next => simp at hstep
    ...
    | setIndex obj idx val =>
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ ho hfuncs_ac hso, hi⟩, hv⟩
        next => simp at hstep
      next addr =>  -- obj = .object addr
        split at hstep
        next =>  -- idx not value
          split at hstep
          next ev' si hsi => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨⟨ho, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hi hfuncs_ac hsi⟩, hv⟩
          next => simp at hstep
        next idxVal =>  -- idx is value
          ...
          next =>  -- val not value → step val
            split at hstep
            next ev' sv hsv => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]; exact ⟨⟨ho, hi⟩, ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hv hfuncs_ac hsv⟩
            next => simp at hstep
      next =>  -- obj = some other value
        split at hstep
        next =>  -- idx not value
    ...
    | deleteProp obj prop =>
      ...
      unfold Flat.step? at hstep
      split at hstep
      next addr hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
      next nobj hv => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp [hasAbruptCompletion]
      next hv =>
        split at hstep
        next ev' so hso => simp [Flat.step?_pushTrace_expand] at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hso
        next => simp at hstep
    | typeof arg =>
      ...
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hsa
        next => simp at hstep
    | call f fenv args =>
      simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
      ...
      | none =>
        rw [Flat.step?.eq_1] at hstep; simp only [hfv] at hstep
        ...
        · next t' sf'' hinner =>
          ...
          exact ⟨⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hf_ac hfuncs_ac hinner, hfe_ac⟩, hargs_ac⟩
        · simp at hstep
      | some fv =>
        cases hev : Flat.exprValue? fenv with
        | none =>
          rw [Flat.step?.eq_1] at hstep; simp only [hfv, hev] at hstep
          simp only [Flat.step?_pushTrace_expand] at hstep
          split at hstep
          · next t' se' hinner =>
            simp only [Option.some.injEq, Prod.mk.injEq] at hstep
        ...
        | some envVal =>
          ...
          · next argVals hvs =>  -- all values
            ...
            · next funcIdx envPtr =>  -- .closure
              split at hstep  -- consoleLog check
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
              · split at hstep  -- funcs[funcIdx]?
                · next funcDef hfd =>  -- normal call → tryCatch wrapper
                  simp at hstep; obtain ⟨_, rfl⟩ := hstep
                  simp [Flat.State.expr, hasAbruptCompletion]
                  exact hfuncs_ac _ funcDef hfd
                · next hfd =>  -- func not found → .lit .undefined
                  simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
            · next =>  -- non-closure → .lit .undefined
              simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
          · next hvs =>  -- not all values → firstNonValueExpr
            split at hstep
            · next done target remaining hfnv =>
              have ⟨htarget, hrem, hrecon⟩ := hasAbruptCompletionList_firstNonValue_preserved hfnv hargs_ac
              split at hstep
              · next t se hse =>
                simp at hstep; obtain ⟨_, rfl⟩ := hstep
                simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]
    ...
    | getEnv envExpr idx =>
      ...
      | none =>
        rw [Flat.step?.eq_1] at hstep; simp only [hev] at hstep
        simp only [Flat.step?_pushTrace_expand] at hstep
        split at hstep
        · next t' se' hinner =>
          simp only [Option.some.injEq, Prod.mk.injEq] at hstep
          ...
          exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hinner
        · simp at hstep
      | some envVal =>
        ...
        simp only [Flat.step?_pushTrace_expand] at hstep
        split at hstep
        next addr => -- .object addr
          split at hstep
          next props hprops => -- heapObjectAt? found
            split at hstep
            next kv hkv => obtain ⟨_, rfl⟩ := hstep; rfl
            next hkv => obtain ⟨_, rfl⟩ := hstep; rfl
          next hprops => obtain ⟨_, rfl⟩ := hstep; rfl
        next naddr henv => -- some non-object
    ...
    | makeEnv vals =>
      ...
      next =>  -- firstNonValueExpr
        ...
        next done target remaining hfnv =>
          ...
          split at hstep
          next t se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueExpr_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_ac hse)
          next => simp at hstep
        next => simp at hstep
    | makeClosure funcIdx envExpr =>
      ...
      next =>  -- envExpr not value → step
        split at hstep
        next ev' se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hac hfuncs_ac hse
        next => simp at hstep
    | objectLit props =>
      simp only [hasAbruptCompletion] at hac
      unfold Flat.step? at hstep
      ...
      next =>  -- firstNonValueProp
        ...
        next done name target remaining hfnv =>
          ...
          split at hstep
          next t se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr, hasAbruptCompletion]; exact hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueProp_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_ac hse)
          next => simp at hstep
        next => simp at hstep
    | arrayLit elems =>
      simp only [hasAbruptCompletion] at hac
        ...
        | none =>
          ...
          · next msg sb hbs =>  -- error
            ...
            · split at hstep  -- isCallFrame (throw)
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
              · -- not isCallFrame: handler = catchBody
                simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp only [Flat.State.expr]; exact hcatch_ac
          · next t sb hbs =>  -- non-error step
            simp at hstep; obtain ⟨_, rfl⟩ := hstep
            simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]
            exact ⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hbody_ac hfuncs_ac hbs, hcatch_ac⟩
          · simp at hstep  -- none
      | some finExpr =>
        simp only [hasAbruptCompletion, Bool.or_eq_false_iff] at hac
        ...
        | none =>
          ...
          · next msg sb hbs =>  -- error
            ...
            · split at hstep
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr, hasAbruptCompletion]
              · -- not isCallFrame: handler = .seq catchBody finExpr
                simp at hstep; obtain ⟨_, rfl⟩ := hstep
                simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]
                exact ⟨hcatch_ac, hfin_ac⟩
          · next t sb hbs =>  -- non-error step
            simp at hstep; obtain ⟨_, rfl⟩ := hstep
            simp only [Flat.State.expr, hasAbruptCompletion, Bool.or_eq_false_iff]
            exact ⟨⟨ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hbody_ac hfuncs_ac hbs, hcatch_ac⟩, hfin_ac⟩
          · simp at hstep  -- none
...
private theorem NoNestedAbrupt_step_preserved (sf sf' : Flat.State) (ev : Core.TraceEvent)
  ...
  induction n with
  | zero =>
    intro e hd env heap trace funcs cs ev sf' hna hfuncs_na hfuncs_ac hstep
    cases e with
    | lit v => simp [Flat.step?] at hstep
  ...
  | succ n ih =>
    ...
    | «continue» _ => unfold Flat.step? at hstep; simp at hstep; obtain ⟨_, rfl⟩ := hstep; exact NoNestedAbrupt.lit
    | unary op arg =>
      cases hna with | unary harg =>
      unfold Flat.step? at hstep
      split at hstep
      next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.unary (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ harg hfuncs_na hfuncs_ac hsa)
        next => simp at hstep
    | binary op l r =>
      cases hna with | binary hlhs hrhs =>
      unfold Flat.step? at hstep
      split at hstep
      next =>  -- lhs not value → step lhs
        split at hstep
        next ev' sl hsl => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.binary (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hlhs hfuncs_na hfuncs_ac hsl) hrhs
        next => simp at hstep
      next lv =>  -- lhs is value
        split at hstep
        next =>  -- rhs not value → step rhs
          split at hstep
          next ev' sr hsr => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.binary NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hrhs hfuncs_na hfuncs_ac hsr)
          next => simp at hstep
        next rv =>  -- both values
          simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
    | while_ cond body =>
      cases hna with | while_ hc hbody =>
      unfold Flat.step? at hstep
      simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
      exact NoNestedAbrupt.if hc (NoNestedAbrupt.seq hbody (NoNestedAbrupt.while_ hc hbody)) NoNestedAbrupt.lit
    | labeled label body =>
      cases hna with | labeled hbody =>
      unfold Flat.step? at hstep
      simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
      exact hbody
    | seq a b =>
      ...
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.seq (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ ha hfuncs_na hfuncs_ac hsa) hb
        next => simp at hstep
    | «let» name init body =>
      cases hna with | «let» hinit hbody =>
      unfold Flat.step? at hstep
      split at hstep
      next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact hbody
      next hv =>
        split at hstep
        next ev' si hsi => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.let (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hinit hfuncs_na hfuncs_ac hsi) hbody
        next => simp at hstep
    | assign name rhs =>
      ...
      next hv =>
        split at hstep
        next ev' sr hsr => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.assign (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hrhs hfuncs_na hfuncs_ac hsr)
        next => simp at hstep
    | «if» cond then_ else_ =>
      ...
      next hv =>
        split at hstep
        next ev' sc hsc => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.if (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hc hfuncs_na hfuncs_ac hsc) hthen helse
        next => simp at hstep
    | throw arg =>
    ...
    | «return» arg =>
      cases hna with
      | return_none => unfold Flat.step? at hstep; simp at hstep; obtain ⟨_, rfl⟩ := hstep; exact NoNestedAbrupt.lit
      | return_some habr =>
        unfold Flat.step? at hstep
        split at hstep
        next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
    ...
    | yield arg d =>
      cases hna with
      | yield_none => unfold Flat.step? at hstep; simp at hstep; obtain ⟨_, rfl⟩ := hstep; exact NoNestedAbrupt.lit
      | yield_some habr =>
        unfold Flat.step? at hstep
        split at hstep
        next v hv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
    ...
    | getProp obj prop =>
      ...
      next hv =>
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.getProp (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hobj hfuncs_na hfuncs_ac hso)
        next => simp at hstep
    | setProp obj prop val =>
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.setProp (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hobj hfuncs_na hfuncs_ac hso) hval
        next => simp at hstep
      next addr =>  -- obj = .object addr
        split at hstep
        next vv =>  -- val is value
        ...
        next =>  -- val not value → step val
          split at hstep
          next ev' sv hsv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.setProp NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hval hfuncs_na hfuncs_ac hsv)
          next => simp at hstep
      next =>  -- obj = some other value
        split at hstep
        next vv =>  -- val is value
    ...
    | getIndex obj idx =>
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.getIndex (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hobj hfuncs_na hfuncs_ac hso) hidx
        next => simp at hstep
      next addr =>  -- obj = .object addr
        split at hstep
        next iv =>  -- idx is value
        ...
        next =>  -- idx not value → step idx
          split at hstep
          next ev' si hsi => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.getIndex NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hidx hfuncs_na hfuncs_ac hsi)
          next => simp at hstep
      next str =>  -- obj = .string str
        split at hstep
        next iv =>  -- idx is value
    ...
    | setIndex obj idx val =>
      ...
      next =>  -- obj not value → step obj
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.setIndex (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hobj hfuncs_na hfuncs_ac hso) hidx hval
        next => simp at hstep
      next addr =>  -- obj = .object addr
        split at hstep
        next =>  -- idx not value → step idx
          split at hstep
          next ev' si hsi => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.setIndex NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hidx hfuncs_na hfuncs_ac hsi) hval
          next => simp at hstep
        next idxVal =>  -- idx is value
          split at hstep
          ...
          next =>  -- val not value → step val
            split at hstep
            next ev' sv hsv => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.setIndex NoNestedAbrupt.lit NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hval hfuncs_na hfuncs_ac hsv)
            next => simp at hstep
    ...
    | deleteProp obj prop =>
      ...
      next hv =>
        split at hstep
        next ev' so hso => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.deleteProp (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hobj hfuncs_na hfuncs_ac hso)
        next => simp at hstep
    | typeof arg =>
      ...
      next hv =>
        split at hstep
        next ev' sa hsa => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.typeof (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ harg hfuncs_na hfuncs_ac hsa)
        next => simp at hstep
    | call f fenv args =>
      ...
      | none =>
        ...
        exact NoNestedAbrupt.call (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hf hfuncs_na hfuncs_ac hinner) henv hargs
      | some fv =>
        cases hev : Flat.exprValue? fenv with
        | none =>
          ...
          obtain ⟨rfl, rfl⟩ := heq
          simp only [Flat.State.expr]
          exact NoNestedAbrupt.call NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ henv hfuncs_na hfuncs_ac hinner) hargs
        | some envVal =>
          rw [Flat.step?.eq_1] at hstep
          simp only [hfv, hev] at hstep
          split at hstep  -- valuesFromExprList?
          ...
          · next hvs =>  -- not all values → firstNonValueExpr
            ...
            · next done target remaining hfnv =>
              ...
              · next t se hse =>
                simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
                exact NoNestedAbrupt.call NoNestedAbrupt.lit NoNestedAbrupt.lit (hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueExpr_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_na hfuncs_ac hse))
              · simp at hstep
            · simp at hstep
    | newObj f fenv args =>
      ...
      | none =>
        ...
        obtain ⟨rfl, rfl⟩ := heq
        simp only [Flat.State.expr]
        exact NoNestedAbrupt.newObj (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hf hfuncs_na hfuncs_ac hinner) henv hargs
      | some fv =>
        cases hev : Flat.exprValue? fenv with
        | none =>
          ...
          obtain ⟨rfl, rfl⟩ := heq
          simp only [Flat.State.expr]
          exact NoNestedAbrupt.newObj NoNestedAbrupt.lit (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ henv hfuncs_na hfuncs_ac hinner) hargs
        | some envVal =>
          rw [Flat.step?.eq_1] at hstep
          simp only [hfv, hev] at hstep
          split at hstep  -- valuesFromExprList?
          ...
          · next hvs =>  -- not all values → firstNonValueExpr
            ...
            · next done target remaining hfnv =>
              ...
              · next t se hse =>
                simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
                exact NoNestedAbrupt.newObj NoNestedAbrupt.lit NoNestedAbrupt.lit (hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueExpr_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_na hfuncs_ac hse))
              · simp at hstep
            · simp at hstep
    | getEnv envExpr idx =>
      ...
      | none =>
        ...
        obtain ⟨rfl, rfl⟩ := heq
        simp only [Flat.State.expr]
        exact NoNestedAbrupt.getEnv (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ henv hfuncs_na hfuncs_ac hinner)
      | some envVal =>
    ...
    | makeEnv vals =>
      ...
      next =>  -- firstNonValueExpr
        ...
        next done target remaining hfnv =>
          ...
          split at hstep
          next t se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.makeEnv (hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueExpr_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_na hfuncs_ac hse))
          next => simp at hstep
        next => simp at hstep
    | makeClosure funcIdx envExpr =>
      ...
      next =>  -- envExpr not value → step
        split at hstep
        next ev' se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.makeClosure (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ henv hfuncs_na hfuncs_ac hse)
        next => simp at hstep
    | objectLit props =>
      ...
      next =>  -- firstNonValueProp
        ...
        next done name target remaining hfnv =>
          ...
          split at hstep
          next t se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.objectLit (hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueProp_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_na hfuncs_ac hse))
          next => simp at hstep
        next => simp at hstep
    | arrayLit elems =>
      ...
      next =>  -- firstNonValueExpr
        ...
        next done target remaining hfnv =>
          ...
          split at hstep
          next t se hse => simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.arrayLit (hrecon _ (ih _ (by simp [Flat.Expr.depth] at hd; have := Flat.firstNonValueExpr_depth hfnv; omega) _ _ _ _ _ _ _ htarget hfuncs_na hfuncs_ac hse))
          next => simp at hstep
        next => simp at hstep
    | tryCatch body param catchBody fin =>
      cases hna with
      | tryCatch_none hbody hcatch =>
        ...
        | none =>
          ...
          · next msg sb hbs =>
            ...
            · split at hstep
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact hcatch
          · next t sb hbs =>
            simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
            exact NoNestedAbrupt.tryCatch_none (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hbody hfuncs_na hfuncs_ac hbs) hcatch
          · simp at hstep
        ...
        | none =>
          ...
          · next msg sb hbs =>
            ...
            · split at hstep
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]; exact NoNestedAbrupt.lit
              · simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
                exact NoNestedAbrupt.seq hcatch hfin
          · next t sb hbs =>
            simp at hstep; obtain ⟨_, rfl⟩ := hstep; simp [Flat.State.expr]
            exact NoNestedAbrupt.tryCatch_some (ih _ (by simp [Flat.Expr.depth] at hd; omega) _ _ _ _ _ _ _ hbody hfuncs_na hfuncs_ac hbs) hcatch hfin
          · simp at hstep
