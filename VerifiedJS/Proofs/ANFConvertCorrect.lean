...
private theorem normalizeExpr_if_cond_source :
  ...
  | zero =>
    refine ⟨?_, ?_, ?_⟩
    · -- normalizeExpr, depth 0
      intro e k hd n m name then_ else_ hk_cond hnorm
      cases e with
      | var vname =>
        simp only [ANF.normalizeExpr] at hnorm
        have := hk_cond (.var vname) n m then_ else_ hnorm
        rw [ANF.Trivial.var.inj this]; exact .var name
      | this =>
        simp only [ANF.normalizeExpr] at hnorm
        have := hk_cond (.var "this") n m then_ else_ hnorm
        rw [ANF.Trivial.var.inj this]; exact .this_var
      | lit v =>
        simp only [ANF.normalizeExpr] at hnorm
        cases ANF.trivialOfFlatValue v with
        | error => exact absurd hnorm (by simp [Throw.throw, MonadExcept.throw, StateT.throw])
        | ok tv =>
          have := hk_cond tv n m then_ else_ hnorm
      ...
      | tryCatch _ _ _ fin => exfalso; cases fin <;> simp [Flat.Expr.depth] at hd
      | _ => exfalso; simp [Flat.Expr.depth] at hd
    · -- normalizeExprList, depth 0
      intro es k hd n m name then_ else_ hk_not hnorm
      cases es with
      | nil => simp only [ANF.normalizeExprList] at hnorm; exact absurd hnorm (hk_not [] n m then_ else_)
      | cons _ _ => exfalso; simp [Flat.Expr.listDepth] at hd
    · -- normalizeProps, depth 0
      intro ps k hd n m name then_ else_ hk_not hnorm
      cases ps with
      | nil => simp only [ANF.normalizeProps] at hnorm; exact absurd hnorm (hk_not [] n m then_ else_)
      | cons _ _ => exfalso; simp [Flat.Expr.propListDepth] at hd
  | succ d ih =>
    obtain ⟨ihe, ihes, ihps⟩ := ih
    refine ⟨?_, ?_, ?_⟩
    · -- normalizeExpr, depth d+1
      intro e k hd n m name then_ else_ hk_cond hnorm
      cases e with
      | var vname =>
        simp only [ANF.normalizeExpr] at hnorm
      ...
      | «return» arg => cases arg with
        | none =>
          exfalso; simp only [ANF.normalizeExpr, pure, Pure.pure, StateT.pure, Except.pure] at hnorm
          exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
        | some _ => sorry -- return (some v): normalizeExpr v K_ret, K_ret never .if
      | yield arg delegate => cases arg with
        | none =>
          exfalso; simp only [ANF.normalizeExpr, pure, Pure.pure, StateT.pure, Except.pure] at hnorm
          exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
        | some _ => sorry -- yield (some v): normalizeExpr v K_yield, K_yield never .if
      | seq a b =>
        simp only [ANF.normalizeExpr] at hnorm
        -- Use Classical.em: does normalizeExpr b k produce .if (.var name)?
        by_cases hb : ∀ (n' m' : Nat) (t' e' : ANF.Expr),
            (ANF.normalizeExpr b k).run n' ≠ .ok (.if (.var name) t' e', m')
        · -- normalizeExpr b k never produces .if (.var name) → name came from a
          have hk_ab : ∀ (arg : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
              ((fun _ => ANF.normalizeExpr b k) arg).run n' = .ok (.if (.var name) t' e', m') → arg = .var name :=
            fun _ n' m' t' e' h => absurd h (hb n' m' t' e')
          exact .seq_l _ _ _ (ihe a _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_ hk_ab hnorm)
        · -- normalizeExpr b k produces .if (.var name) → name came from b
          push_neg at hb
          obtain ⟨n', m', t', e', hb'⟩ := hb
          exact .seq_r _ _ _ (ihe b k (by simp [Flat.Expr.depth] at hd; omega) n' m' name t' e' hk_cond hb')
      | «if» c t e =>
        simp only [ANF.normalizeExpr] at hnorm
        -- K_if satisfies condition: K_if condTriv = do ...; pure (.if condTriv ...)
        -- If output is .if (.var name) ..., condition is condTriv = .var name = arg ✓
        have hk_if : ∀ (arg : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            ((fun condTriv => do
              let thenExpr ← ANF.normalizeExpr t k
              let elseExpr ← ANF.normalizeExpr e k
              pure (.if condTriv thenExpr elseExpr)) arg).run n' = .ok (.if (.var name) t' e', m') →
            arg = .var name := by
          intro arg n' m' t' e' h
          simp only [bind, Bind.bind, StateT.bind, Except.bind, pure, Pure.pure, StateT.pure, Except.pure] at h
          split at h <;> [exact nomatch h; skip]
          split at h <;> [exact nomatch h; skip]
          exact ANF.Trivial.var.inj (ANF.Expr.if.inj (Prod.mk.inj (Except.ok.inj h)).1).1
        exact .if_cond _ _ _ _ (ihe c _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_ hk_if hnorm)
      | assign name' value =>
        simp only [ANF.normalizeExpr] at hnorm
        -- bindComplex continuation never produces .if → name came from value
        have hk_bc : ∀ (arg : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            (ANF.bindComplex (.assign name' arg) k).run n' ≠ .ok (.if (.var name) t' e', m') :=
          fun arg => bindComplex_not_if _ k
        have hk_assign : ∀ (arg : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            ((fun vTriv => ANF.bindComplex (.assign name' vTriv) k) arg).run n' =
            .ok (.if (.var name) t' e', m') → arg = .var name :=
          fun arg n' m' t' e' h => absurd h (hk_bc arg n' m' t' e')
        exact .assign_value _ _ _ (ihe value _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_ hk_assign hnorm)
      | throw arg =>
        simp only [ANF.normalizeExpr] at hnorm
        have hk_throw : ∀ (a : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            (pure (ANF.Expr.throw a) : ANF.ConvM ANF.Expr).run n' ≠ .ok (.if (.var name) t' e', m') := by
          intro a n' m' t' e'; simp [pure, Pure.pure, StateT.pure, Except.pure]
          intro h; exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj h)).1
        have hk_t : ∀ (a : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            ((fun argTriv => pure (ANF.Expr.throw argTriv)) a).run n' = .ok (.if (.var name) t' e', m') → a = .var name :=
          fun a n' m' t' e' h => absurd h (hk_throw a n' m' t' e')
        exact .throw_arg _ _ (ihe arg _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_ hk_t hnorm)
      | await arg =>
        simp only [ANF.normalizeExpr] at hnorm
        have hk_aw : ∀ (a : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            ((fun argTriv => pure (ANF.Expr.await argTriv)) a).run n' ≠ .ok (.if (.var name) t' e', m') := by
          intro a n' m' t' e'; simp [pure, Pure.pure, StateT.pure, Except.pure]
          intro h; exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj h)).1
        exact .await_arg _ _ (ihe arg _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (hk_aw a n' m' t' e')) hnorm)
      | typeof arg =>
        simp only [ANF.normalizeExpr] at hnorm
        have hk_bc : ∀ (a : ANF.Trivial) (n' m' : Nat) (t' e' : ANF.Expr),
            ((fun argTriv => ANF.bindComplex (.typeof argTriv) k) a).run n' ≠ .ok (.if (.var name) t' e', m') :=
          fun a => bindComplex_not_if _ k
        exact .typeof_arg _ _ (ihe arg _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (hk_bc a n' m' t' e')) hnorm)
      | unary op arg =>
        simp only [ANF.normalizeExpr] at hnorm
        exact .unary_arg _ _ _ (ihe arg _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (bindComplex_not_if _ k n' m' name t' e')) hnorm)
      | getProp obj prop =>
        simp only [ANF.normalizeExpr] at hnorm
        exact .getProp_obj _ _ _ (ihe obj _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (bindComplex_not_if _ k n' m' name t' e')) hnorm)
      | deleteProp obj prop =>
        simp only [ANF.normalizeExpr] at hnorm
        exact .deleteProp_obj _ _ _ (ihe obj _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (bindComplex_not_if _ k n' m' name t' e')) hnorm)
      | getEnv envPtr idx =>
        simp only [ANF.normalizeExpr] at hnorm
        exact .getEnv_env _ _ _ (ihe envPtr _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (bindComplex_not_if _ k n' m' name t' e')) hnorm)
      | makeClosure funcIdx env =>
        simp only [ANF.normalizeExpr] at hnorm
        exact .makeClosure_env _ _ _ (ihe env _ (by simp [Flat.Expr.depth] at hd; omega) n m name then_ else_
          (fun a n' m' t' e' h => absurd h (bindComplex_not_if _ k n' m' name t' e')) hnorm)
      | _ => sorry -- remaining compound cases: let, labeled, while_, tryCatch, setProp, getIndex,
                   -- setIndex, binary, call, newObj, makeEnv, arrayLit, objectLit
    · sorry -- normalizeExprList
    · sorry -- normalizeProps
