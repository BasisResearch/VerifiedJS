...
private theorem normalizeExpr_if_step_sim
    ...
    ∃ (sf' : Flat.State) (evs : List Core.TraceEvent),
      ...
      ExprWellFormed sf'.expr sf'.env := by
  subst hheap henv
  -- Case split on sf.expr to understand the Flat expression structure
  cases hsf : sf.expr with
  | «if» fc ft fe =>
    rw [hsf] at hnorm hewf
    -- Unfold ANF step and branch
    unfold ANF.step? at hstep_eq
    simp only [ANF.pushTrace] at hstep_eq
    -- Case split on fc (the Flat condition expression)
    cases fc with
    | var name_fc =>
      -- normalizeExpr (.if (.var name_fc) ft fe) k: cond = .var name_fc
      simp only [ANF.normalizeExpr] at hnorm
      simp only [bind, Bind.bind, StateT.bind, StateT.run, Except.bind] at hnorm
      split at hnorm
      · cases hnorm
      · rename_i hft_pair hft_ok
        ...
        · rename_i hfe_pair hfe_ok
          ...
          have ⟨hif_eq, hm_eq⟩ := Prod.mk.inj (Except.ok.inj hnorm)
          have ⟨hcond_eq, hthen_eq, helse_eq⟩ := ANF.Expr.if.inj hif_eq
          subst hcond_eq hthen_eq helse_eq hm_eq
          -- Now cond = .var name_fc, then_ = fst of hft_pair, else_ = fst of hfe_pair
          -- hft_ok : (normalizeExpr ft k).run n = .ok hft_pair
          -- hfe_ok : (normalizeExpr fe k).run hft_pair.2 = .ok hfe_pair
          split at hstep_eq
          · -- evalTrivial env (.var name_fc) = .ok v
            rename_i v heval
            simp only [ANF.evalTrivial] at heval
            split at heval
            · rename_i val hlookup
              have hv := Except.ok.inj heval; subst hv
              split at hstep_eq
              · -- toBoolean val = true → step to then_
                obtain ⟨rfl, rfl⟩ := hstep_eq
                rename_i hbool_anf
                -- Use steps_if_var_branch for Flat stepping
                have hval_sf : sf.env.lookup name_fc = some val := hlookup
                obtain ⟨sf', hsteps, hexpr', henv', hheap', htrace'⟩ :=
                  steps_if_var_branch { sf with expr := .«if» (.var name_fc) ft fe } name_fc val ft fe hval_sf
                have hsteps_sf : Flat.Steps sf [.silent, .silent] sf' := by
                  have : sf = { sf with expr := .«if» (.var name_fc) ft fe } := by cases sf; simp [hsf]
                  rw [this]; exact hsteps
                have hbool_flat : Flat.toBoolean val = true := by
                  cases val <;> simp_all [Flat.toBoolean]
                rw [hbool_flat, if_true] at hexpr'
                refine ⟨sf', [.silent, .silent], hsteps_sf, ?_, ?_, ?_⟩
                · -- observableTrace [.silent] = observableTrace [.silent, .silent]
                  simp [observableTrace]; decide
                · -- ANF_SimRel
                  refine ⟨?_, ?_, ?_, k, n, hft_pair.snd, ?_, hk⟩
                  · rw [hheap']
                  · rw [henv']
                  · rw [htrace']; simp [observableTrace_append]; rw [← htrace]
                    simp [observableTrace]; decide
                  · rw [hexpr']; exact hft_ok
                · -- ExprWellFormed
                  rw [hexpr']
                  rw [hsf] at hewf
                  intro x hfx; exact hewf x (.if_then _ _ _ _ hfx)
              · -- toBoolean val = false → step to else_
                obtain ⟨rfl, rfl⟩ := hstep_eq
                rename_i hbool_anf
                have hval_sf : sf.env.lookup name_fc = some val := hlookup
                obtain ⟨sf', hsteps, hexpr', henv', hheap', htrace'⟩ :=
                ...
                have hsteps_sf : Flat.Steps sf [.silent, .silent] sf' := by
                  have : sf = { sf with expr := .«if» (.var name_fc) ft fe } := by cases sf; simp [hsf]
                  rw [this]; exact hsteps
                have hbool_flat : Flat.toBoolean val = false := by
                  cases val <;> simp_all [Flat.toBoolean]
                rw [hbool_flat, if_false] at hexpr'
                refine ⟨sf', [.silent, .silent], hsteps_sf, ?_, ?_, ?_⟩
                · simp [observableTrace]; decide
                · refine ⟨?_, ?_, ?_, k, hft_pair.snd, hfe_pair.snd, ?_, hk⟩
                  · rw [hheap']
                  · rw [henv']
                  · rw [htrace']; simp [observableTrace_append]; rw [← htrace]
                    simp [observableTrace]; decide
                  · rw [hexpr']; exact hfe_ok
                · rw [hexpr']
                  rw [hsf] at hewf
                  intro x hfx; exact hewf x (.if_else _ _ _ _ hfx)
            · -- env.lookup name_fc = none → error
              rename_i hnone
              simp at heval
          · -- evalTrivial env (.var name_fc) = .error msg
            rename_i msg herr
            obtain ⟨rfl, rfl⟩ := hstep_eq
            exfalso
            ...
            · rename_i hnone
              rw [hsf] at hewf
              exact hnone (hewf name_fc (.if_cond _ _ _ _ (.var name_fc)))
    | lit v_flat =>
      simp only [ANF.normalizeExpr] at hnorm
      -- trivialOfFlatValue v_flat must succeed; unfold and handle
      rw [trivialOfFlatValue_eq_trivialOfValue] at hnorm
      simp only [bind, Bind.bind, StateT.bind, StateT.run, Except.bind] at hnorm
      split at hnorm
      · cases hnorm
        ...
        · rename_i hfe_pair hfe_ok
          ...
          · rename_i v heval
            ...
            · -- true branch
              ...
              rename_i hbool_anf
              obtain ⟨sf', hsteps, hexpr', henv', hheap', htrace'⟩ :=
                steps_if_lit_branch { sf with expr := .«if» (.lit v_flat) ft fe } v_flat ft fe
              have hsteps_sf : Flat.Steps sf [.silent] sf' := by
                have : sf = { sf with expr := .«if» (.lit v_flat) ft fe } := by cases sf; simp [hsf]
                rw [this]; exact hsteps
              -- Connect ANF and Flat toBoolean
              have heval_val : v = v_flat := by
                simp [ANF.evalTrivial, ANF.trivialValue?, ANF.trivialOfValue] at heval
                cases v_flat <;> simp_all [ANF.trivialOfValue, ANF.trivialValue?]
              subst heval_val
              have hbool_flat : Flat.toBoolean v_flat = true := by
                cases v_flat <;> simp_all [Flat.toBoolean]
              rw [hbool_flat, if_true] at hexpr'
              refine ⟨sf', [.silent], hsteps_sf, ?_, ?_, ?_⟩
              · simp [observableTrace]; decide
              · refine ⟨?_, ?_, ?_, k, n, hft_pair.snd, ?_, hk⟩
                · rw [hheap']
                · rw [henv']
            ...
            · -- false branch
              ...
              have heval_val : v = v_flat := by
                ...
                cases v_flat <;> simp_all [ANF.trivialOfValue, ANF.trivialValue?]
              subst heval_val
              have hbool_flat : Flat.toBoolean v_flat = false := by
                cases v_flat <;> simp_all [Flat.toBoolean]
              rw [hbool_flat, if_false] at hexpr'
              refine ⟨sf', [.silent], hsteps_sf, ?_, ?_, ?_⟩
              · simp [observableTrace]; decide
              ...
              · rw [hexpr']
                rw [hsf] at hewf
                intro x hfx; exact hewf x (.if_else _ _ _ _ hfx)
          · -- error: evalTrivial on a literal value never fails
            rename_i msg herr
            exfalso
            simp [ANF.evalTrivial, ANF.trivialValue?, ANF.trivialOfValue] at herr
            cases v_flat <;> simp_all [ANF.trivialOfValue, ANF.trivialValue?]
    | «this» =>
      simp only [ANF.normalizeExpr] at hnorm
      simp only [bind, Bind.bind, StateT.bind, StateT.run, Except.bind] at hnorm
      ...
      · rename_i hft_pair hft_ok
        ...
        · rename_i hfe_pair hfe_ok
          ...
          split at hstep_eq
          · rename_i v heval
            -- evalTrivial env (.var "this") = .ok v means env.lookup "this" = some v
            simp only [ANF.evalTrivial] at heval
            split at heval
            · rename_i val hlookup
              have hv := Except.ok.inj heval; subst hv
              ...
              · -- true branch
                obtain ⟨rfl, rfl⟩ := hstep_eq
                rename_i hbool_anf
                -- For .this: Flat needs to resolve "this" then branch
                -- Use steps_if_var_branch with name = "this"
                have hval_sf : sf.env.lookup "this" = some val := hlookup
                obtain ⟨sf', hsteps, hexpr', henv', hheap', htrace'⟩ :=
                  steps_if_var_branch { sf with expr := .«if» .this ft fe } "this" val ft fe
                    (by simp [Flat.step?, Flat.exprValue?]; exact hval_sf)
                sorry -- this case needs steps_if_this_branch adaptation; defer
              · -- false branch
                obtain ⟨rfl, rfl⟩ := hstep_eq
                sorry -- symmetric to true case
            · rename_i hnone; simp at heval
          · rename_i msg herr
            obtain ⟨rfl, rfl⟩ := hstep_eq
            exfalso
            simp only [ANF.evalTrivial] at herr
            ...
            · rename_i hnone
              rw [hsf] at hewf
              exact hnone (hewf "this" (.if_cond _ _ _ _ .this_var))
    | seq a_fc b_fc =>
      sorry -- Complex condition: seq chain in if condition; needs trivialChain machinery
    | _ =>
      sorry -- Other complex fc: bindComplex cases can't produce .if (needs normalizeExpr_not_if_family)
  | seq a b =>
    rw [hsf] at hnorm hewf
    sorry -- seq case: needs inductive stepping through seq prefix
  | var name_sf =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr] at hnorm
    obtain ⟨m', hk'⟩ := hk (.var name_sf) n
    rw [hk'] at hnorm; exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
  | lit v_sf =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr] at hnorm
    rw [trivialOfFlatValue_eq_trivialOfValue] at hnorm
    obtain ⟨m', hk'⟩ := hk (ANF.trivialOfValue v_sf) n
    rw [hk'] at hnorm; exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
  | «this» =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr] at hnorm
  ...
  | «break» _ | «continue» _ =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr, pure, Pure.pure, StateT.pure, Except.pure] at hnorm
    exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
  | «return» arg =>
    rw [hsf] at hnorm
    cases arg with
    | none =>
      simp only [ANF.normalizeExpr, pure, Pure.pure, StateT.pure, Except.pure] at hnorm
      exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
    | some _ => sorry -- needs normalizeExpr_not_if for .return (some ..)
  | yield arg delegate =>
    rw [hsf] at hnorm
    cases arg with
    | none =>
      simp only [ANF.normalizeExpr, pure, Pure.pure, StateT.pure, Except.pure] at hnorm
      exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
    | some _ => sorry -- needs normalizeExpr_not_if for yield
  | labeled _ body =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr, bind, Bind.bind, StateT.bind, Except.bind] at hnorm
    split at hnorm
    · cases hnorm
    · simp only [pure, Pure.pure, StateT.pure, Except.pure] at hnorm
      exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1
  | while_ wc wb =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr, bind, Bind.bind, StateT.bind, Except.bind] at hnorm
    repeat (first | split at hnorm | cases hnorm)
    all_goals (try simp only [pure, Pure.pure, StateT.pure, Except.pure] at hnorm)
    all_goals (try exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1)
    all_goals sorry
  | tryCatch tbody tcp tcb tfin =>
    rw [hsf] at hnorm
    simp only [ANF.normalizeExpr, bind, Bind.bind, StateT.bind, Except.bind] at hnorm
    cases tfin with
    | none =>
      ...
      all_goals (try exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1)
    | some fin =>
      simp only [Functor.map, StateT.map, Except.bind] at hnorm
      repeat (first | split at hnorm | cases hnorm)
      all_goals (try simp only [pure, Pure.pure, StateT.pure, Except.pure] at hnorm)
      all_goals (try exact ANF.Expr.noConfusion (Prod.mk.inj (Except.ok.inj hnorm)).1)
  | _ => sorry -- remaining cases: need normalizeExpr_not_if_family
