            ...
            all_goals first | exact hf.elim | (
              ...
              | [v1] =>
                ...
                have hs2 : ∃ wv, s2.stack = [wv] := by
                  cases s2.stack with
                  | nil => simp at hlen
                  | cons wv rest => cases rest with | nil => exact ⟨wv, rfl⟩ | cons => simp at hlen; omega
                obtain ⟨wv, hs2⟩ := hs2
                simp only [traceToWasm]; exact ⟨_, by simp [step?, hcw, hs2, withI32Bin, withI32Rel, pop2?, trapState, pushTrace],
                  { hemit := hrel.hemit, hcode := .nil, hstack := hrel.hstack,
                    hframes_len := hrel.hframes_len, hframes_locals := hrel.hframes_locals,
                    hframes_vals := hrel.hframes_vals, hglobals := hrel.hglobals, hmemory := hrel.hmemory, hmemLimits := hrel.hmemLimits, hmemory_aligned := hrel.hmemory_aligned, hmemory_nonempty := hrel.hmemory_nonempty,
                    hlabels := hrel.hlabels, hhalt := hhalt_of_structural (@EmitCodeCorr.nil (s1.labels.map (·.name))) hrel.hlabels,
                    hlabel_content := hrel.hlabel_content,
          ...
          | .f64 =>
            ...
            all_goals first | exact hf.elim | (
              ...
              | [] =>
                ...
                match hstk_w : s2.stack with
                | [] =>
                  simp only [traceToWasm]; exact ⟨_, by simp [step?, hcw, hstk_w, withF64Bin, pop2?, trapState, pushTrace],
                    { hemit := hrel.hemit, hcode := .nil, hstack := by simp [hstk_w],
                      hframes_len := hrel.hframes_len, hframes_locals := hrel.hframes_locals,
                      hframes_vals := hrel.hframes_vals, hglobals := hrel.hglobals, hmemory := hrel.hmemory, hmemLimits := hrel.hmemLimits, hmemory_aligned := hrel.hmemory_aligned, hmemory_nonempty := hrel.hmemory_nonempty,
                      hlabels := hrel.hlabels, hhalt := hhalt_of_structural (@EmitCodeCorr.nil (s1.labels.map (·.name))) hrel.hlabels,
                      hlabel_content := hrel.hlabel_content,
              ...
              | [v1] =>
                ...
                have hs2 : ∃ wv, s2.stack = [wv] := by
                  ...
                  | cons wv rest => cases rest with | nil => exact ⟨wv, rfl⟩ | cons => simp at hlen; omega
                obtain ⟨wv, hs2⟩ := hs2
                simp only [traceToWasm]; exact ⟨_, by simp [step?, hcw, hs2, withF64Bin, pop2?, trapState, pushTrace],
                  { hemit := hrel.hemit, hcode := .nil, hstack := hrel.hstack,
                    hframes_len := hrel.hframes_len, hframes_locals := hrel.hframes_locals,
                    hframes_vals := hrel.hframes_vals, hglobals := hrel.hglobals, hmemory := hrel.hmemory, hmemLimits := hrel.hmemLimits, hmemory_aligned := hrel.hmemory_aligned, hmemory_nonempty := hrel.hmemory_nonempty,
                    hlabels := hrel.hlabels, hhalt := hhalt_of_structural (@EmitCodeCorr.nil (s1.labels.map (·.name))) hrel.hlabels,
