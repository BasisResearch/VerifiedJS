...
private theorem ByteArray.size_set! (a : ByteArray) (i : Nat) (v : UInt8) :
  ...
  simp only [ByteArray.set!, ByteArray.size]
...
/-! ## binOp trap step lemmas: when pop2? fails, withI32Bin/withI32Rel/withF64Bin trap -/

private theorem step?_eq_i32Add_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Add :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.add") := by
  cases s; simp_all [step?, withI32Bin, trapState, pushTrace]

private theorem step?_eq_i32Sub_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Sub :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.sub") := by
  cases s; simp_all [step?, withI32Bin, trapState, pushTrace]

private theorem step?_eq_i32Mul_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Mul :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.mul") := by
  cases s; simp_all [step?, withI32Bin, trapState, pushTrace]

private theorem step?_eq_i32And_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32And :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.and") := by
  cases s; simp_all [step?, withI32Bin, trapState, pushTrace]

private theorem step?_eq_i32Or_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Or :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.or") := by
  cases s; simp_all [step?, withI32Bin, trapState, pushTrace]

private theorem step?_eq_i32Eq_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Eq :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.eq") := by
  cases s; simp_all [step?, withI32Rel, trapState, pushTrace]

private theorem step?_eq_i32Ne_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Ne :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.ne") := by
  cases s; simp_all [step?, withI32Rel, trapState, pushTrace]

private theorem step?_eq_i32Lts_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Lts :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.lt_s") := by
  cases s; simp_all [step?, withI32Rel, trapState, pushTrace]

private theorem step?_eq_i32Gts_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.i32Gts :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in i32.gt_s") := by
  cases s; simp_all [step?, withI32Rel, trapState, pushTrace]

private theorem step?_eq_f64Add_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.f64Add :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in f64.add") := by
  cases s; simp_all [step?, withF64Bin, trapState, pushTrace]

private theorem step?_eq_f64Sub_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.f64Sub :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in f64.sub") := by
  cases s; simp_all [step?, withF64Bin, trapState, pushTrace]

private theorem step?_eq_f64Mul_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.f64Mul :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in f64.mul") := by
  cases s; simp_all [step?, withF64Bin, trapState, pushTrace]

private theorem step?_eq_f64Div_trap (s : ExecState) (rest : List Instr)
    (hcode : s.code = Instr.f64Div :: rest) (hpop : pop2? s.stack = none) :
    step? s = some (trapState { s with code := rest } "type mismatch in f64.div") := by
  cases s; simp_all [step?, withF64Bin, trapState, pushTrace]

/-! ## binOp type mismatch trap lemmas: when pop2? returns non-matching types -/

/-- withI32Bin traps when the top two stack values aren't both i32. -/
private theorem withI32Bin_type_mismatch (s : ExecState) (op : UInt32 → UInt32 → UInt32)
    (name : String) (a b : WasmValue) (stk : List WasmValue)
    (hstack : s.stack = a :: b :: stk)
    (hmis : ∀ x y, (a, b) ≠ (.i32 x, .i32 y)) :
    withI32Bin s op name = some (trapState s ("type mismatch in " ++ name)) := by
  simp [withI32Bin, pop2?, hstack]
  cases a <;> cases b <;> simp_all [trapState, pushTrace]

/-- withI32Rel traps when the top two stack values aren't both i32. -/
private theorem withI32Rel_type_mismatch (s : ExecState) (op : UInt32 → UInt32 → Bool)
    (name : String) (a b : WasmValue) (stk : List WasmValue)
    (hstack : s.stack = a :: b :: stk)
    (hmis : ∀ x y, (a, b) ≠ (.i32 x, .i32 y)) :
...
private theorem withF64Bin_type_mismatch (s : ExecState) (op : Float → Float → Float)
    ...
    withF64Bin s op name = some (trapState s ("type mismatch in " ++ name)) := by
  simp [withF64Bin, pop2?, hstack]
  cases a <;> cases b <;> simp_all [trapState, pushTrace]

private theorem step?_eq_i32Add_type_mismatch (s : ExecState) (rest : List Instr)
...
theorem irMultiStep_i32Const_drop (s : IRExecState) (v : String) (n : Nat) (rest : List IRInstr)
  ...
  have hstep1 := IRStep_of_irStep? (irStep?_eq_i32Const s v n (.drop :: rest) hcode hv)
  have hstep2 := IRStep_of_irStep? (irStep?_eq_drop
    { s with code := IRInstr.drop :: rest,
             stack := .i32 n.toUInt32 :: s.stack,
             trace := s.trace ++ [TraceEvent.silent] }
    rest (.i32 n.toUInt32) s.stack rfl rfl)
  exact ⟨_, .tail hstep1 (.tail hstep2 (.refl _)), rfl, rfl, rfl, rfl, rfl, rfl⟩
...
theorem irMultiStep_f64Const_drop (s : IRExecState) (v : String) (rest : List IRInstr)
    ...
    ∃ s' : IRExecState, IRSteps s [TraceEvent.silent, TraceEvent.silent] s' ∧
      ...
      s'.frames = s.frames ∧ s'.labels = s.labels ∧
      s'.module = s.module ∧ s'.globals = s.globals := by
  have hstep1 := IRStep_of_irStep? (irStep?_eq_f64Const s v (.drop :: rest) hcode)
  have hstep2 := IRStep_of_irStep? (irStep?_eq_drop
    { s with code := IRInstr.drop :: rest,
             stack := .f64 (v.toNat?.map (fun m => Float.ofNat m) |>.getD 0.0) :: s.stack,
             trace := s.trace ++ [TraceEvent.silent] }
    rest (.f64 (v.toNat?.map (fun m => Float.ofNat m) |>.getD 0.0)) s.stack rfl rfl)
  exact ⟨_, .tail hstep1 (.tail hstep2 (.refl _)), rfl, rfl, rfl, rfl, rfl, rfl⟩
...
/-- If-case macro step: given LowerSimRel with expr = if cond then_ else_,
    the IR can multi-step to match any ANF step on the if expression.
    Internally: executes condCode via irMultiStep_trivialCode, then call truthy + if_,
    entering the chosen branch. The if-block label is absorbed by the axiom
    (the branch code generated by lowering does not br out of the if block). -/
axiom irMultiStep_ifCase (prog : ANF.Program) (irmod : IRModule)
    (s1 : ANF.State) (s2 : IRExecState)
    (cond : ANF.Trivial) (then_ else_ : ANF.Expr)
    (ct : Core.TraceEvent) (s1' : ANF.State)
    (hrel : LowerSimRel prog irmod s1 s2)
    (hexpr : s1.expr = .«if» cond then_ else_)
...
axiom irMultiStep_yieldCase (prog : ANF.Program) (irmod : IRModule)
    (s1 : ANF.State) (s2 : IRExecState)
    (arg : Option ANF.Trivial) (delegate : Bool)
    (ct : Core.TraceEvent) (s1' : ANF.State)
    (hrel : LowerSimRel prog irmod s1 s2)
...
theorem step_sim (prog : ANF.Program) (irmod : IRModule) :
  ...
  · rename_i heq
    ...
    | .«if» cond then_ else_ =>
        ...
        -- Proved via irMultiStep_ifCase axiom (absorbs block label).
    ...
    | .await arg =>
        ...
        -- Phase 1: Execute argCode via irMultiStep_trivialCode.
        -- Phase 2: Execute call awaitOp via irMultiStep_awaitOp.
        have hc := hrel.hcode; rw [hexpr] at hc
        -- Get code form from lowering constraint
        obtain ⟨argCode, hcode_eq, htcc⟩ :=
          lower_await_of_labels_empty prog irmod s1 s2 arg
            hrel.hlower hc hrel.hlabels_empty
        -- Show evalTrivial succeeds
        have hscope : ∀ name, arg = .var name → ∃ v, s1.env.lookup name = some v := by
          intro name harg; rw [harg] at hexpr; exact hrel.hawait_var_scope name hexpr
        obtain ⟨v, heval⟩ := evalTrivial_ok_of_var_scope s1.env arg hscope
        -- Derive the ANF step result (await ok → .silent trace)
        have hs1_eta : { s1 with expr := ANF.Expr.await arg } = s1 := by cases s1; simp_all
        have hanf := ANF.step?_await_ok s1 arg v heval
        rw [hs1_eta] at hanf; rw [hanf] at heq
        simp only [Option.some.injEq, Prod.mk.injEq] at heq
        obtain ⟨rfl, rfl⟩ := heq
        ...
        -- Phase 2: step through call awaitOp
        obtain ⟨s2', trace2, hsteps2, hcode', hlbl', hfr', hmod'', hobs2⟩ :=
          irMultiStep_awaitOp s2_mid hcode_mid
            (by rw [hfr_mid]; exact hrel.hframes_one)
            (by rw [hlbl_mid]; exact hrel.hlabels_empty)
        -- Combine IR steps
        refine ⟨s2', trace1 ++ trace2, IRSteps_trans hsteps1 hsteps2, ?_, ?_⟩
        ...
        -- Observable events match: traceFromCore .silent = .silent, observableEvents [.silent] = []
        · simp [observableEvents_append, hobs1, hobs2]
    | .labeled label body =>
  ...
  | instr :: rest =>
            ...
            · -- Wasm code = i32Store {offset, align:2} :: rest_w
              ...
              | .i32 val :: .i32 addr :: stk =>
                ...
                | some mem' =>
                  ...
                  rcases hrel.hmemory with hmem_eq | ⟨hmem_none, hmem_sz⟩
                  · -- Memory exists
                    have hbind : Option.bind s2.store.memories[0]? (fun m => writeLE? m (addr.toNat + offset) 4 val.toUInt64) = some mem' := by
                ...
                | none =>
                  ...
                  rcases hrel.hmemory with hmem_eq | ⟨hmem_none, _⟩
                  · -- Memory exists but OOB
                    have hwrite_w : Option.bind s2.store.memories[0]? (fun m => writeLE? m (addr.toNat + offset) 4 val.toUInt64) = none := by
                      rw [hmem_eq]; simp [hwrite]
          ...
          · match hstk : s1.stack with
            ...
            | .i32 val :: .i32 addr :: stk =>
              ...
              | some mem' =>
                ...
                obtain ⟨wstk', hstack_eq, hlen_tail, htail⟩ := hstk_w
                rcases hrel.hmemory with hmem_eq | ⟨hmem_none, hmem_sz⟩
                · have hbind : Option.bind s2.store.memories[0]? (fun m => writeLE? m (addr.toNat + offset) 1 val.toUInt64) = some mem' := by
                    rw [hmem_eq]; simp [hwrite]
              ...
              | none =>
                ...
                obtain ⟨wstk', hstack_eq, _, _⟩ := hstk_w
                rcases hrel.hmemory with hmem_eq | ⟨hmem_none, _⟩
                · have hwrite_w : Option.bind s2.store.memories[0]? (fun m => writeLE? m (addr.toNat + offset) 1 val.toUInt64) = none := by
                    rw [hmem_eq]; simp [hwrite]
                  have hw : step? s2 = some (.trap ("memory access fault in i32.store"),
                      { s2 with code := [], trace := s2.trace ++ [.trap ("memory access fault in i32.store")] }) := by
                    simp [step?, hcw, hstack_eq, pop2?, hwrite_w, trapState, pushTrace]
            ...
            all_goals first | exact hf.elim | (
              ...
              | [v1] =>
                ...
                exact ⟨_, hw, {
                  hemit := hrel.hemit, hcode := .nil,
                  hstack := by
                    have h1 := hrel.hstack.1; rw [hstk] at h1
                    refine ⟨h1, fun i hi => ?_⟩
                    have h := hrel.hstack.2 i (by rw [hstk]; exact hi)
                    rw [hstk] at h; exact h,
                  hframes_len := hrel.hframes_len, hframes_locals := hrel.hframes_locals,
                  hframes_vals := hrel.hframes_vals, hglobals := hrel.hglobals,
                  hmemory := hrel.hmemory, hmemLimits := hrel.hmemLimits,
                  hmemory_aligned := hrel.hmemory_aligned, hmemory_nonempty := hrel.hmemory_nonempty,
      ...
      | .br label =>
          ...
          · -- Wasm code = br idx :: rest_w
            ...
            -- hlabel_content for tail indices (in drop(idx+1))
          ...
          · -- Wasm code = brIf idx :: rest_w
            ...
            | [] =>
              ...
              have hir : irStep? s1 = some (.trap "stack underflow in br_if",
                  { s1 with code := [], trace := s1.trace ++ [.trap "stack underflow in br_if"] }) := by
                simp [irStep?, hcode_ir, hstk, irPop1?, irTrapState, irPushTrace]
              rw [hir] at hstep
              simp only [Option.some.injEq, Prod.mk.injEq] at hstep
              obtain ⟨rfl, rfl⟩ := hstep
              ...
              refine ⟨_, hw, ⟨hrel.hemit, ?_, ?_, hrel.hframes_len, hrel.hframes_locals,
                ...
                hrel.hmemory_aligned, hrel.hmemory_nonempty, hrel.hlabels, ?_,
            ...
            | .i32 cond :: stk =>
              ...
              · -- True branch: branch to label (cond ≠ 0)
                ...
                | wv :: wstk =>
                  ...
                  cases hval_corr with
                  | i32 n =>
                    have hw := step?_eq_brIf_true_gen s2 idx rest_w cond wstk wLbl
                      (if wLbl.isLoop then wLbl :: s2.labels.drop (idx + 1)
                       else s2.labels.drop (idx + 1))
                      hcw hs2 h0 hresolve
                    have hloop_eq : irLbl.isLoop = wLbl.isLoop := hloop
            ...
            | .i64 _ :: stk =>
              ...
              have hir : irStep? s1 = some (.trap "br_if condition is not i32",
                  { s1 with code := [], trace := s1.trace ++ [.trap "br_if condition is not i32"] }) := by
                simp [irStep?, hcode_ir, hstk, irPop1?, irTrapState, irPushTrace]
              rw [hir] at hstep
              simp only [Option.some.injEq, Prod.mk.injEq] at hstep
              obtain ⟨rfl, rfl⟩ := hstep
              ...
              | wv :: wstk =>
                have hval_corr := hrel.hstack.2 0 (by simp [hstk])
                rw [hstk, hs2] at hval_corr; simp at hval_corr
                cases hval_corr with
                | i64 m =>
...
theorem ir_forward_sim (prog : ANF.Program) (irmod : IRModule)
  ...
  · exact LowerSimRel.init prog irmod hlower (lower_main_code_corr prog irmod hlower)
      (lower_main_var_scope prog irmod hlower)
  · exact {
...
/-- Stuttering version: lower_behavioral_correct using IRStutterSim. -/
